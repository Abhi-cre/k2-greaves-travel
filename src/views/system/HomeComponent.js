...
class HomeComponent extends React.Component {
  ...
  getMailList = async (folderId) => {
    ...
    if (ArrayHelper.getValue(responseInboxList, "IsSuccess") == true) {
      this.setState({
        fetchMails: ArrayHelper.getValue(responseInboxList, "Mails"),
        displayNext:
          ArrayHelper.getValue(responseInboxList, "Mails").length >=
      ...
      if (this.state.page > 1) {
        let selectedFolderData = folderList.filter(
          (item) => item.displayName == this.state.displayName
        )[0];
        let selectedMail = ArrayHelper.getValue(
          selectedFolderData,
          "mailLists",
          []
        );
        mailLists = selectedMail.concat(
          ArrayHelper.getValue(responseInboxList, "Mails")
        );
      }
  ...
  getFolderList = async () => {
    ...
    if (ArrayHelper.getValue(responseGetFolderList, "IsSuccess") == true) {
      let folderList = ArrayHelper.getValue(
        responseGetFolderList,
        "MailFolders",
        []
      );
      ...
      this.setState({
        folderList: folderList,
        folderId: ArrayHelper.getValue(inboxData, "[0].id"),
      });
  ...
  selectedMail = async (e1, e2, e3) => {
    ...
    if (e1.hasAttachments == true) {
      this.setState({ loader: true });
      let responseattachmentList = await OutLookApi.MailAttachments(
        ArrayHelper.getValue(e1, "id")
      );
      ...
      if (ArrayHelper.getValue(responseattachmentList, "IsSuccess") == true) {
        e1 = Object.assign({}, e1, {
          attechmentLists: ArrayHelper.getValue(
  ...
  getTourRecordData = async (messageId) => {
    ...
    let response = await SettingApi.PostSettingList(
      data,
      "/api/TourRecord/TourRecordByMessageId"
    );
    if (
      ArrayHelper.getValue(response, "isSuccess") == true &&
      ArrayHelper.getValue(response, "tourRecordMail", []).length > 0
    ) {
      this.setState({
        loader: false,
        tourRecordData: ArrayHelper.getValue(response, "tourRecordMail[0]"),
      });
    } else {
      this.setState({ loader: false });
      let mailDetail = ArrayHelper.getValue(
        this.state.mailDetail,
      ...
      if (mailDetail.length > 2) {
        this.setState({
          tourRecordData: {
            tourName: mailDetail[2],
            tourRecordId: mailDetail[1],
            id: mailDetail[3],
            vendorId: mailDetail[4],
          },
        });
      }
    }
  ...
  selecteFolder(e) {
    ...
    this.setState({
      ...
      folderId: e.id,
      page: 1,
      displayNext: ArrayHelper.getValue(e, "mailLists", []) > 0 ? true : false,
      selectedEmailLists: ArrayHelper.getValue(e, "mailLists", []),
    });
  ...
  nextPrev(str) {
    ...
    setTimeout(() => {
      this.setState({ loader: false });
      if (
        numEmail >= numRecordEmail &&
        numRecordEmail >= this.state.mailCount
      ) {
  ...
  deleteMails = async (mailId = "") => {
    ...
    if (this.state.displayName == "Deleted Items") {
      ...
      this.setState({ mailDetail: {}, page: 1, selectedEmailLists: [] });
      this.moveMailToFolder(
        "AQMkAGY5OGQ2YzVhLTBlNDMtNDE0MC04NTM3LWFjYWRiYWU1NzM2MAAuAAADEbuoBCLHZUKF805QFgUJ9gEAKmPw4QC3cE6U10G80KfPyQAAAgEKAAAA",
        mailId
      );
    }
  };
  ...
  searchingEmail = async (e1, e2) => {
    ...
    if (e2 == "SearchMailFromMailAddress") {
      ...
      urlLink =
        "SearchMailHasAttachment?attachmentName=" + e1 + "&hasAttachment=true";
    ...
    if (ArrayHelper.getValue(responseSearching, "IsSuccess") == true) {
      this.setState({
        loader: false,
        serachBy: e2,
        selectedEmailLists: ArrayHelper.getValue(responseSearching, "Mails"),
      });
    }
  };
  /* For Delete Mail Function Start */
  ...
  MarkReadEmail = async () => {
    ...
    if (selectedIdList.length > 0) {
      ...
      if (
        ArrayHelper.getValue(responseSearching, "IsSuccess") == true ||
        ArrayHelper.getValue(responseSearching, "IsSuccess") == false
      ) {
  ...
  nextPrevEmail = async (e, e2) => {
    ...
    let selectedEmailLists = await selectedEmailListsAll.filter(
      (item, key) => key == e
    );
    ...
    this.setState({
      selectedEmailLists: selectedEmailListsAll,
      mailDetail: {
  ...
  getTourRecordList = async () => {
    ...
    let response = await SettingApi.PostSettingList(
      data,
      "/api/TourRecord/TourRecordOfNullMailId"
    );
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      this.setState({
        loader: false,
        tourRecordList: ArrayHelper.getValue(response, "tourRecordMail"),
      });
    }
  };
  /* For Mark Read Email Function Email */
    ...
    return (
      <React.Fragment>
        ...
        <LeftNevigationBarComponent
          ...
          selecteFolder={(e) => this.selecteFolder(e)}
        ...
        <div
          className={`page-content ${
            this.props.showLeftNav != true ? "removeSpace" : ""
          }`}
        >
          ...
          <EmailListsComponent
            ...
            increaseDescriseCount={(str, str2) =>
              this.increaseDescriseCount(str, str2)
            }
            page={this.state.page}
            mailCount={this.state.mailCount}
          ...
          <EmailDetailComponent
            deleteMails={(mailId) => this.deleteMails(mailId)}
          ...
          />
          {this.props.disPlayRight != "assignToToure" ? (
            <EmailFormComponent
...
const mapDispatchToProps = (dispatch) => {
  return {
    folderList: (dataval) =>
      dispatch({ type: actionTypes.FOLDER_LIST, payload: dataval }),
    selectedEmailLists: (dataval) =>
      dispatch({ type: actionTypes.SELECTED_EMAIL_LIST, payload: dataval }),
  };
};
