...
class EmailFormComponent extends React.Component {
  ...
  appendEmailSignature = (emailContent) => {
    const storedUserName = localStorage.getItem("__user_name__");
    const storedUserEmail = localStorage.getItem("__user_email__");
    const storedDepartment = localStorage.getItem("__department_name__");
    const emailSignature = `
      <p style="color: #333; font-size: 14px;">
         <strong>${storedUserName}</strong><br>
         Email: ${storedUserEmail}<br>
        Department: ${storedDepartment}<br>
    
      <a href="https://www.greavestours.com/" target="_blank">Visit our website</a><br>
        <img src="https://www.greavestours.com/Greaves-tours-signature.png" alt="Company Logo" style="max-width: 200px; margin-top: 10px;" />
      
      </p>
    `;

    // Append the signature to the email content
    return emailContent + emailSignature;
  };

  // Update the state and set initial email content along with the signature
  componentDidMount() {
    const storedUserName = localStorage.getItem("__user_name__");
    const storedUserEmail = localStorage.getItem("__user_email__");
    const storedDepartment = localStorage.getItem("__department_name__");

    console.log("User Name from LocalStorage:", storedUserName);
    ...
    `;
    // Set the signature in the state, email content starts empty
    this.setState({ emailSignature, emailContent: "" });
  }
  handleChange = (e) => {
  ...
  submit = async (event: any) => {
    ...
    if (
      this.state.mailFormType != "compose" &&
      ArrayHelper.getValue(this.props.tourRecordData, "tourRecordId") != ""
    ) {
      ...
      if (vendorTypeEmail.length < 2) {
        ...
        subject = ArrayHelper.getValue(
          ...
          "/[ref: TEF" +
            ArrayHelper.getValue(this.props.tourRecordData, "id") +
            "]",
          ""
        );
        emailExtraContent =
        ...
        subject = ArrayHelper.getValue(
          ...
          "/[ref: VEF" +
            ArrayHelper.getValue(this.props.tourRecordData, "vendorId") +
            "]",
          ""
        );
        emailExtraContent =
  ...
  componentWillReceiveProps(nextProps, oldProps) {
    if (nextProps.disPlayRight != this.props.disPlayRight) {
      this.setState({
        displayCcEmail: false,
        displayBccEmail: false,
        mailFormType: nextProps.disPlayRight,
        emailId: ArrayHelper.getValue(nextProps.mailDetail, "mailData.id"),
        toEmail: ArrayHelper.getValue(
          nextProps.mailDetail,
          "mailData.from.emailAddress.address"
        ),
      });
      if (
        nextProps.disPlayRight == "forward" ||
        nextProps.disPlayRight == "compose"
      ) {
      ...
      <React.Fragment>
        ...
        <div
          className={`content-right ${
            this.props.disPlayRight == "compose" ||
            this.props.disPlayRight == "single" ||
            this.props.disPlayRight == "all" ||
            this.props.disPlayRight == "forward"
              ? "show"
              : "hide"
          } `}
        >
            ...
            <form method="post" role="form" onSubmit={this.submit}>
              ...
              <div className="mail-compose">
                <div className="form-group bordered-left-4 bordered-themeprimary">
                  ...
                  {this.state.mailFormType == "forward" ||
                  this.state.mailFormType == "compose" ? (
                    <input
                      required
                      name="toEmail"
                ...
                <div className="form-group bordered-left-4 bordered-themesecondary">
                  ...
                  <div className="field-options">
                    {this.props.ImportanceType == "High" ? (
                      <a>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: this.state.emailSignature,
                  }}
                />
                ...
                <div className="d-flex mailSend">
                  ...
                  this.props.disPlayRight == "forward" ? (
                    <button
                      onClick={() => this.DiscardMail()}
                      type="button"
                      className="btn btn-outlined me-4"
                    >
                      Discard
                    </button>
                  ) : (
