...
class ToursViewRecordComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,selectedTab:'Tours',toursSelectedData:{},TourItineraryData:[],TourItinerarySelected:{},TourItineraryMaster:{},quotationList:[],PassengerData:[],MessagesList:[],NotesList:[],FlightData:[],PaymentData:[],VendorPaymentData:[],vendorList:[],serviceList:[],perPersonCost:[]};
    }
    componentWillMount()
    {
    ...
    getServiceList=async()=>{
        ...
        else
        {
            ...
            if(ArrayHelper.getValue(response,'isSuccess')== true)
            {
              ...
              this.setState({loader:false,serviceList:ArrayHelper.getValue(response,'services')});
    ...
    getItinerary=async()=>
    {
        ...
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          if(ArrayHelper.getValue(response,'tourItinerary').length>0)
          {
            ...
            let TourItinerarySelected=ArrayHelper.getValue(response,'tourItinerary').filter((item)=>item.isPrimary==true);

            if(TourItinerarySelected.length>0)
            {
                this.setState({TourItinerarySelected:TourItinerarySelected[0],TourItineraryMaster:TourItinerarySelected[0]});  
            }
            else
            {
                this.setState({TourItinerarySelected:ArrayHelper.getValue(response,'tourItinerary')[0],TourItineraryMaster:ArrayHelper.getValue(response,'tourItinerary')[0]});  
            }
    ...
    getQuatationList()
    {
      ...
      if(ArrayHelper.getValue(this.state.TourItinerarySelected,'tourItineraryService').length>0)
      {
        let quotationList=[]
        ArrayHelper.getValue(this.state.TourItinerarySelected,'tourItineraryService').map((item)=>{
    ...
    getPassengerList=async(id)=>
    {
        ...
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          this.setState({loader:false,PassengerData:ArrayHelper.getValue(response,'tourPassengers')});
    ...
    getFlightList=async(id)=>
    {
        ...
        let data ={
          ...
          let response= await SettingApi.PostSettingList(data,'/api/TourFlights/FlightListByTourId');
           if(ArrayHelper.getValue(response,'isSuccess')== true)
           {
             this.setState({loader:false,FlightData:ArrayHelper.getValue(response,'tourFlights')});
    ...
    getPaymentList=async(id)=>
    {
        ...
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          this.setState({loader:false,PaymentData:ArrayHelper.getValue(response,'tourPayments')});
    ...
    getVendorPaymentList=async(id)=>
    {
        ...
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          this.setState({loader:false,VendorPaymentData:ArrayHelper.getValue(response,'vendorPayments')});
        ...
        return(
            ...
            <React.Fragment>
                    ...
                    <div className="row navbar navbar-expand-lg ">
                        <div className="col-md-8 navbar-collapse">
                            <ul className="navbar-nav">
                                ...
                                {(this.state.TourItineraryData.length>0)?<li className="nav-item dropdown">
                                    <a onClick={()=>this.selectTab('Itinerary')} className={`nav-link dropdown-toggle costing ${(this.state.selectedTab=='Itinerary')?'active':''}`}>
                                    Itinerary
                                    </a>
                                    <ul id="Itinerary" className="dropdown-menu">
                                        {this.state.TourItineraryData.map((item,key)=>{
                                            return( <li key={`Itinerary-${key}`}>
                                                <a className="dropdown-item" onClick={()=>this.selectItenary(item,'Itinerary')}> {item.name}</a>
                                            </li>)
                                        })}
                                    </ul>
                                </li>:''}
                                ...
                                {/* {(this.state.TourItineraryData.length>0)?<li className="nav-item">
                        ...
                        <div className="col-md-4 d-sm-inline-flex align-items-center justify-content-end" onMouseOver={()=>{$(".dropdown-menu").removeClass('show')}}>
                           
                            {(this.state.selectedTab=='Tours')?<div className="w-auto pt-2 pb-4 me-2">
                            <NavLink to="/tours/update-record"> <button type="button" className="btn btn-outlined rounded">Edit</button></NavLink>
                            </div>:''}
                           
                            <div className="w-auto pt-2 pb-4">
                            <NavLink to="/tours">   <button type="button" className="btn btn-primary rounded"><i className="fa-sharp fa-solid fa-arrow-right"></i>    Tours List</button></NavLink>
                            </div>
                        </div>
                    </div>
                </div>
                <ToursViewComponent 
                  ...
                  />  
                  {(this.state.TourItineraryData.length>0)?<TourItineraryViewComponent 
                  selectedTab={this.state.selectedTab}
                 ...
                 />:''} 
                {(this.state.TourItineraryData.length>0)?<CostingViewComponent perPersonCost={this.state.perPersonCost}  serviceList={this.state.serviceList}  selectedTab={this.state.selectedTab}  TourItinerarySelected={this.state.TourItinerarySelected} currency={ArrayHelper.getValue(this.props.toursSelectedData,'currency')} displayType='View' />:''}
                                
                  {(this.state.TourItineraryData.length>0)?<QuotationViewComponent perPersonCost={this.state.perPersonCost} quotationList={this.state.quotationList}  selectedTab={this.state.selectedTab}  TourItinerarySelected={this.state.TourItinerarySelected} currency={ArrayHelper.getValue(this.props.toursSelectedData,'currency')} />:''}
                  <VendorMailComposeComponent   vendorList={this.state.vendorList}  toursData={this.props.toursSelectedData}   selectedTab={this.state.selectedTab}/>
                  {(this.state.TourItineraryData.length>0)?<ClientMailLogViewComponent vendorList={this.state.vendorList} selectedTab={this.state.selectedTab}  toursData={this.props.toursSelectedData}/>:''}
                  {(this.state.TourItineraryData.length>0)?<VendorMailLogViewComponent vendorList={this.state.vendorList} selectedTab={this.state.selectedTab}  toursData={this.props.toursSelectedData}/>:''}
                <div className="text-end p-4">
               
                {(this.state.selectedTab=='Tours')?<NavLink to="/tours/update-record"> <button type="button" className="btn btn-outlined rounded">Edit</button></NavLink>:''}
...
const mapStateToProps = state => {
    console.log(state.settingsData.toursSelected,"toursSelectedDatatoursSelectedData");
...
export default  connect(mapStateToProps, mapDispatchToProps)(ToursViewRecordComponent);