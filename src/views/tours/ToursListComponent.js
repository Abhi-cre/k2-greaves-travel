...
class ToursListComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    if (toursListData.length > 0) {
      ...
      this.setState({
        toursListAll: toursListData,
        toursListFilter: toursListData,
        toursList: toursList,
      });
    } else {
      this.gettoursList();
  ...
  gettoursList = async () => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      ...
      for (let i = 0; i < this.state.perPage; i++) {
        if (
          ArrayHelper.getValue(
            ArrayHelper.getValue(response, "tourRecords")[i],
            "id"
          ) != ""
        ) {
      ...
      this.setState({
        loader: false,
        toursListAll: ArrayHelper.getValue(response, "tourRecords"),
        toursListFilter: ArrayHelper.getValue(response, "tourRecords"),
        toursList: toursList,
      });
  ...
  ToursDelete = async (str) => {
    if (window.confirm("Do you want to delete selected Tours?")) {
      ...
      if (ArrayHelper.getValue(response, "isSuccess") == true) {
        ...
        this.setState({
          loader: false,
          toursListAll: toursListAll,
          toursListFilter: toursListAll,
          toursList: toursList,
        });
        setTimeout(() => {
  ...
  filterTours = async () => {
    ...
    if (
      ...
      let toursListFilter = toursListAll.filter((item) => {
        if (
          item.tourName.search(new RegExp(this.state.keyword.trim(), "i")) !=
            -1 &&
          this.state.keyword.trim() != ""
        ) {
          return item;
        } else if (
          item.tourRecordId.search(
            new RegExp(this.state.keyword.trim(), "i")
          ) != -1 &&
          this.state.keyword.trim() != ""
        ) {
          return item;
  ...
  clearTours() {
    ...
    this.setState({
      ...
      endDate: "",
      currentPage: 1,
      toursListFilter: this.state.toursListAll,
    });
    setTimeout(() => {
      this.setState({ loader: false });
      this.props.history("/tours");
        ...
        <div className="p-4 ms-2">
          <div className="row mb-3">
            ...
            <div className="col-md-4 text-end">
              ...
              {ALLOWEDUSER.filter(
                (item: any) => item == localStorage.getItem(USER_EMAIL)
              ).length > 0 || ISSITESTATUS == "live" ? (
                <NavLink to="/tours/add-record">
                  {" "}
                  <button className="btn btn-primary rounded">
                    <i className="fa-sharp fa-solid fa-plus"></i> Add Tour
                    Record
                  </button>
                </NavLink>
              ) : (
                ...
                <table className="table table-striped rounded border">
                  <tbody>
                    ...
                    {this.state.toursList.map((item, key) => {
                      ...
                      if (
                        ArrayHelper.getValue(item, "greavesStatusName") ==
                        "Confirmed"
                      ) {
                      ...
                      if (
                        ArrayHelper.getValue(item, "clientStatusName") ==
                        "Confirmed"
                      ) {
                      ...
                      return (
                        <tr key={`toursList-${key}`}>
                          ...
                          <td>
                            {ArrayHelper.getValue(item, "gtinConsultantName")}
                          </td>
                          <td>
                            {ArrayHelper.getValue(item, "gtusConsultantName")}
                          </td>
                          <td>
                            <span className={`badge ${greavesStatusClass}`}>
                              {ArrayHelper.getValue(item, "greavesStatusName")}
                            </span>
                          </td>
                          <td>
                            <span className={`badge ${clientStatusClass}`}>
                              {ArrayHelper.getValue(item, "clientStatusName")}
                            </span>
                          </td>
                          <td>{ArrayHelper.getValue(item, "createdByName")}</td>
                          <td>
                            {ArrayHelper.getValue(item, "createdOn")
                              ? formatDate(
                                  ArrayHelper.getValue(item, "createdOn"),
                                  "dd MMM yyyy"
                                )
                          ...
                          {ALLOWEDUSER.filter(
                            ...
                            <td>
                              ...
                              <button
                                onClick={() => this.ToursSelected(item, "edit")}
                              >
                                <svg
                                  fill="#000000"
                                  width="18"
                          ...
                          <td>
                            <i
                              onClick={() => this.ToursSelected(item, "view")}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    toursSelectedInfo: (data) =>
      dispatch({ type: actionTypesUser.TOURS_SELECTED, payload: data }),
  };
};
