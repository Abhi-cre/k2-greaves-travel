        ...
        return(
            ...
            <React.Fragment>
                <div className={`border-box ${(this.props.selectedTab!='Itinerary')?'hide':''}`}>
                    ...
                    return(<div key={`internary-${key}`} className="accordion border-0" id={`accordionExample-${key}`}>
{/*                         
                            ...
                            {item.name}
                              ...
                              {(item.id!=0)?<div className="col-auto  txt-right text-danger cursourPointer" onClick={() => this.props.deleteItinerary(key,item.id)}><button type="button" className="btn removeInternary rounded"><i className="fa-solid fa-trash"></i> Itinerary Remove</button></div>:''} 
