...
class ToursAddRecordComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      messageType: "",
      formDisply: true,
      isImportItinerary: false,
      selectedItineraryList: [],
      selectedTourId: "",
      isItenaryServiceUpdate: "",
      displayType: "byCost",
    };
  }
  componentWillMount() {
  ...
  addTours() {
    ...
    toursData.push({
      ...
      traveler_home_phone: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "tourContact[0].homePhone"
      ),
      traveler_office_phone: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "tourContact[0].officePhone"
      ),
      traveler_cell: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "tourContact[0].personalNo"
      ),
      traveler_socialLink: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "tourContact[0].socialLink"
      ),
      id:
        ArrayHelper.getValue(this.props.toursSelectedData, "id") != ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "id")
          : 0,
      ...
      leadValue: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "leadValue"
      ),
      agencyId:
        ArrayHelper.getValue(this.props.toursSelectedData, "agencyId") != ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "agencyId")
          : 0,
      agentId:
        ArrayHelper.getValue(this.props.toursSelectedData, "agentId") != ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "agentId")
          : 0,
      gtusConsultantId:
        ArrayHelper.getValue(
      ...
      gtinConsultantId:
        ArrayHelper.getValue(
          ...
          : 0,
      tourStartDate:
        ArrayHelper.getValue(this.props.toursSelectedData, "tourStartDate") !=
        ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "tourStartDate")
          : "",
      tourEndDate:
        ArrayHelper.getValue(this.props.toursSelectedData, "tourEndDate") != ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "tourEndDate")
          : "",
      tourTypeId:
        ArrayHelper.getValue(this.props.toursSelectedData, "tourTypeId") != ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "tourTypeId")
          : 0,
      statusFromGreavesId:
        ArrayHelper.getValue(
      ...
      statusFromClientId:
        ArrayHelper.getValue(
          ...
          : 0,
      noOfPax:
        ArrayHelper.getValue(this.props.toursSelectedData, "noOfPax") != ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "noOfPax")
          : "1",
      paymentLastDate: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "paymentLastDate"
      ),
      confirmationDate: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "confirmationDate"
      ),
      currency: ArrayHelper.getValue(this.props.toursSelectedData, "currency"),
      fileUSDROE:
        ArrayHelper.getValue(this.props.toursSelectedData, "fileUSDROE") != ""
          ? ArrayHelper.getValue(this.props.toursSelectedData, "fileUSDROE")
          : 0,
      tourRouting: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "tourRouting"
      ),
      salesRegionId: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "salesRegionId"
      ),
      wetuLink: ArrayHelper.getValue(this.props.toursSelectedData, "wetuLink"),
      hearedAbout: ArrayHelper.getValue(
        this.props.toursSelectedData,
        "hearedAbout"
      ),
    });
  ...
  getQuatationList() {
    if (
      ArrayHelper.getValue(
        this.state.TourItineraryData,
        "[0].tourItineraryService"
      ).length > 0
    ) {
      let quotationList = [];
      ArrayHelper.getValue(
      ...
      if (ArrayHelper.getValue(response, "tourItinerary").length > 0) {
        ...
        tourItinerary = tourItinerary.map((item) => {
          if (item.tourItineraryService.length > 0) {
            item.tourItineraryService = item.tourItineraryService.map(
              (_item) => {
                let vendorList = this.state.vendorList;
                vendorList = vendorList.filter(
                  (__item) => __item.vendorTypeId == _item.vendorTypeId
                );
                let serviceList = this.state.serviceList;
                serviceList = serviceList.filter(
                  (__item) => __item.vendorId == _item.vendorId
                );
                _item = Object.assign({}, _item, {
                  vendorList: vendorList,
                  serviceList: serviceList,
                  commissionType:
                    _item.markupPercentage > 0 ? "byPercentage" : "byAmount",
                  commissionTypeUS:
                    _item.serviceUSDMarkupPercentage > 0
                      ? "byPercentage"
                      : "byAmount",
                });
                return _item;
        ...
        let TourItineraryData = tourItinerary.filter(
          (item) => item.isPrimary == true
        );
        if (this.state.selectedItenaryId != "") {
          this.setState({
            TourItineraryData: tourItinerary.filter(
              (_item: any) => _item.id == this.state.selectedItenaryId
            ),
          });
          ...
          ArrayHelper.getValue(response, "tourItinerary").length == 1
        ) {
  ...
  addPassenger(e: any) {
    ...
    if (
      ArrayHelper.getValue(this.state.toursData, "[0].id") != "" &&
      ArrayHelper.getValue(this.state.toursData, "[0].id") != 0
    ) {
      this.setState({ loader: true });
      setTimeout(() => {
        this.postPassenger(
  ...
  postFlight = async (tourdId) => {
    ...
    if (FlightData.length > 0) {
      FlightData.map(async (item, key) => {
        ...
        item.departureDate =
          item.departureDate != "" &&
          item.departureDate != "1900-01-01" &&
          item.departureDate != undefined
            ? formatDate(item.departureDate, "yyyy-MM-dd")
            : "1900-01-01T00:00:00";
        item.arrivalDate =
          item.arrivalDate != "" &&
          item.arrivalDate != "1900-01-01" &&
          item.arrivalDate != undefined
            ? formatDate(item.arrivalDate, "yyyy-MM-dd")
            : "1900-01-01T00:00:00";
        ...
        if (item.airLineId != "") {
          ...
          if (item.id == 0) {
            this.setState({ loader: true });
            response = await SettingApi.PostSettingList(
              postdata,
              "/api/TourFlights/Add"
            );
          } else {
  ...
  getFlightList = async (id) => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      this.setState({
        loader: false,
        FlightList: ArrayHelper.getValue(response, "tourFlights"),
      });
  ...
  postPayment = async (tourdId) => {
    ...
    if (PaymentData.length > 0) {
      PaymentData.map(async (item, key) => {
        ...
        item.paymentDate =
          item.paymentDate != "" &&
          item.paymentDate != "1900-01-01" &&
          item.paymentDate != undefined
            ? formatDate(item.paymentDate, "yyyy-MM-dd")
            : "1900-01-01T00:00:00";
        item.dueDate =
          item.dueDate != "" &&
          item.dueDate != "1900-01-01" &&
          item.dueDate != undefined
            ? formatDate(item.dueDate, "yyyy-MM-dd")
            : "1900-01-01T00:00:00";
        if (item.amount != "") {
          let postdata = {
            requestedUserId: parseInt(localStorage.getItem(USER_ID)),
  ...
  getPaymentList = async (id) => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      this.setState({
        loader: false,
        PaymentList: ArrayHelper.getValue(response, "tourPayments"),
      });
  ...
  deleteVendorPayment = async (_key, e3) => {
    if (e3 != 0 && e3 != "") {
      if (window.confirm("Do you want to delete selected Vendor Payment?")) {
        ...
        VendorPaymentList = VendorPaymentList.filter(
          (item, key) => key != _key
        );
    ...
    if (VendorPaymentData.length > 0) {
      VendorPaymentData.map(async (item, key) => {
        ...
        item.transactionDate =
          item.transactionDate != "" &&
          item.transactionDate != "1900-01-01" &&
          item.transactionDate != undefined
            ? formatDate(item.transactionDate, "yyyy-MM-dd")
            : "1900-01-01T00:00:00";
        item.transactionDueDate =
          item.transactionDueDate != "" &&
          item.transactionDueDate != "1900-01-01" &&
          item.transactionDueDate != undefined
            ? formatDate(item.transactionDueDate, "yyyy-MM-dd")
            : "1900-01-01T00:00:00";
        if (item.amount != "") {
          let postdata = {
            requestedUserId: parseInt(localStorage.getItem(USER_ID)),
  ...
  getVendorPaymentList = async (id) => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      this.setState({
        loader: false,
        VendorPaymentList: ArrayHelper.getValue(response, "vendorPayments"),
      });
    ...
    if (
      ...
      setTimeout(async () => {
        ...
        bodyContent =
          bodyContent +
          "Tour Record id " +
          ArrayHelper.getValue(this.state.toursData, "[0].tourRecordId") +
          " has a new Message added by " +
        ...
        let sendMail = await SettingApi.PostTaveleList(
          mailData,
          "/api/bifrost/Azure/SendMail"
        );
  ...
  deleteItinerary = async (_key, e3) => {
    if (window.confirm("Do you want to delete selected Tours Itinerary?")) {
      ...
      if (e3 != 0 && e3 != "") {
        ...
        let response = await SettingApi.PostSettingList(
          data,
          "/api/TourItinerary/DeleteItinerary"
        );
        if (ArrayHelper.getValue(response, "isSuccess") == true) {
  ...
  addItinerarySerice(str) {
    let serviceGSTPercentage = ArrayHelper.getValue(
      this.state.appSettingList.filter((_item) => _item.settingName == "GST"),
      "[0].settingValue"
    );
    this.setState({ serviceGSTPercentage: serviceGSTPercentage });
    if (str == "") {
      let TourItineraryServiceData = [];
      TourItineraryServiceData = TourItineraryServiceData.concat([
        {
          vendorList: [],
          serviceList: [],
          id: 0,
          ...
          markupAmount: 0.0,
          isShowOnItinerary: 0.0,
          serviceGTICommission: 0.0,
          serviceGrossINR: 0.0,
          serviceGSTPercentage: serviceGSTPercentage,
          serviceSellINR: 0.0,
          serviceNetUSD: 0.0,
          serviceUSDMarkupPercentage: 0.0,
          serviceUSDCommission: 0.0,
          serviceUSDClientDollar: 0.0,
          creditCardFees: 0.0,
          quoteType: 0,
          commissionType: "byPercentage",
          commissionTypeUS: "byPercentage",
          serviceUSDMarkUpAmount: 0,
          startDate: "",
          endDate: "",
          origin: "",
          destination: "",
          ...
          noofAdult: 0,
          noofChild: 0,
          agentCommissionType: "byPercentage",
      ...
      if (this.state.isItenaryServiceUpdate != "yes") {
        ...
        TourItineraryServiceData[0].rate = 0.0;
        TourItineraryServiceData[0].rateType = "";
        TourItineraryServiceData[0].description = "";
        TourItineraryServiceData[0].unit = 1;
        TourItineraryServiceData[0].cost = 0.0;
        TourItineraryServiceData[0].markupAmount = 0.0;
        TourItineraryServiceData[0].isShowOnItinerary = 0.0;
        TourItineraryServiceData[0].serviceGTICommission = 0.0;
        TourItineraryServiceData[0].serviceGrossINR = 0.0;
        TourItineraryServiceData[0].serviceGSTPercentage = serviceGSTPercentage;
        TourItineraryServiceData[0].serviceSellINR = 0.0;
        TourItineraryServiceData[0].serviceNetUSD = 0.0;
        TourItineraryServiceData[0].serviceUSDCommission = 0.0;
        TourItineraryServiceData[0].serviceUSDClientDollar = 0.0;
        ...
        TourItineraryServiceData[0].taxAmount = 0.0;
  ...
  postItinerarySerice = async (e1, e2, e3) => {
    ...
    if (
      this.state.TourItineraryServiceData[0].id != 0 &&
      this.state.TourItineraryServiceData[0].id != "" &&
      this.state.isImportItinerary == false
    ) {
      tourItineraryService = tourItineraryService.filter(
        (item) => item.id != this.state.TourItineraryServiceData[0].id
      );
      ...
      let response = await SettingApi.PostSettingList(
        data,
        "/api/TourItinerary/DeleteTourItineraryService"
      );
      if (ArrayHelper.getValue(response, "isSuccess") == true) {
        this.setState({ loader: false });
      }
    ...
    TourItineraryServiceData[0].vendorTypeName = ArrayHelper.getValue(
      this.state.vendorTypeList.filter(
        (_it: any) => _it.id == TourItineraryServiceData[0].vendorTypeId
      ),
      "[0].name"
    );
    TourItineraryServiceData[0].vendorName = ArrayHelper.getValue(
      this.state.vendorList.filter(
        (_it: any) => _it.id == TourItineraryServiceData[0].vendorId
      ),
      "[0].vendorName"
    );
    TourItineraryServiceData[0].serviceName = ArrayHelper.getValue(
      this.state.serviceList.filter(
        (_it: any) => _it.id == TourItineraryServiceData[0].serviceId
      ),
      "[0].name"
    );
    TourItineraryServiceData[0].quoteTypeName = ArrayHelper.getValue(
      this.state.quoteTypeList.filter(
        (_it: any) => _it.id == TourItineraryServiceData[0].quoteType
      ),
      "[0].name"
    );
    tourItineraryService = tourItineraryService.concat(
      TourItineraryServiceData
    );
  ...
  deleteItinerarySerice = async (e1, e2, e3) => {
    if (e3 == 0 || this.state.isImportItinerary == true) {
      ...
      let tourItineraryService = state[e1].tourItineraryService;
      tourItineraryService = tourItineraryService.filter(
        (_: number, key: number) => key != e2
      );
      state[e1].tourItineraryService = tourItineraryService;
      this.setState({ TourItineraryData: state });
    } else if (
      window.confirm("Do you want to delete selected Tours Itinerary Service?")
    ) {
      const state = this.state.TourItineraryData;
      let tourItineraryService = state[e1].tourItineraryService;
      tourItineraryService = tourItineraryService.filter(
  ...
  selectTab(e) {
    if (
      e == "Itinerary" &&
      (ArrayHelper.getValue(this.state.toursData, "[0].fileUSDROE") == "" ||
        ArrayHelper.getValue(this.state.toursData, "[0].fileUSDROE") == "0" ||
        ArrayHelper.getValue(this.state.toursData, "[0].tourStartDate") == "" ||
        ArrayHelper.getValue(this.state.toursData, "[0].tourEndDate") == "" ||
        ArrayHelper.getValue(this.state.toursData, "[0].tourName") == "")
    ) {
  ...
  postIntenary = async (tourdId) => {
    ...
    TourItineraryData.map(async (item) => {
      ...
      item.itineraryUSMarkupAmount = await parseInt(
        item.itineraryUSMarkupAmount
      );
      item.itineraryCreditCardFees = await parseFloat(
        item.itineraryCreditCardFees
      );
      ...
      item.tourItineraryService = await item.tourItineraryService.map(
        (_item) => {
          ...
          _item.serviceFeesId =
            _item.serviceFeesId != "" ? parseInt(_item.serviceFeesId) : 0;
          ...
          if (_item.id == 0) {
            ...
            _item.serviceUSDMarkupPercentage =
              _item.commissionTypeUS == "byPercentage"
                ? parseFloat(_item.serviceUSDMarkupPercentage)
                : 0;
          }
          _item.agentCommissionByPercentage =
            _item.agentCommissionByPercentage != ""
              ? parseFloat(_item.agentCommissionByPercentage)
              : 0.0;
          _item.agentCommissionValue =
            _item.agentCommissionValue != ""
              ? parseFloat(_item.agentCommissionValue)
              : 0.0;
          ...
          _item.serviceUSDClientDollar = parseFloat(
            _item.serviceUSDClientDollar
          );
          _item.creditCardFees = parseFloat(_item.creditCardFees);
          _item.quoteType = parseInt(_item.quoteType);
          //_item.serviceId= parseInt(_item.serviceId);
          //_item.serviceId= parseInt(_item.serviceId);
          _item.isShowOnItinerary =
            _item.isShowOnItinerary == "Yes" ? true : false;
          _item.startDate =
            ArrayHelper.getValue(_item, "startDate") != "0001-01-01T00:00:00" &&
            ArrayHelper.getValue(_item, "startDate") != "0001-01-01" &&
            ArrayHelper.getValue(_item, "startDate") != ""
              ? `${formatDate(
                  ArrayHelper.getValue(_item, "startDate"),
                  "yyyy-MM-dd"
                )}T${formatDate(
              ...
              : "1900-01-01T00:00:00";
          _item.endDate =
            ArrayHelper.getValue(_item, "endDate") != "0001-01-01T00:00:00" &&
            ArrayHelper.getValue(_item, "endDate") != "0001-01-01" &&
            ArrayHelper.getValue(_item, "endDate") != ""
              ? `${formatDate(
                  ArrayHelper.getValue(_item, "endDate"),
                  "yyyy-MM-dd"
                )}T${formatDate(
      ...
      if (item.id != 0 && item.id != "" && item.name != "") {
        ...
        let response = await SettingApi.PostSettingList(
          data,
          "/api/TourItinerary/AddTourItinerary"
        );
        if (ArrayHelper.getValue(response, "isSuccess") == true) {
          if (ArrayHelper.getValue(this.props.toursSelectedData, "id") != "") {
            this.getItinerary();
            let TourItineraryData = [
              ArrayHelper.getValue(response, "tourItinerary")[
                ArrayHelper.getValue(response, "tourItinerary").length - 1
              ],
            ];
            ...
            this.setState({
              loader: false,
              AllTourItineraryData: ArrayHelper.getValue(
                response,
                "tourItinerary"
              ),
              TourItineraryData: TourItineraryData,
            });
            setTimeout(() => {
              this.addItinerarySerice("");
            }, 1);
  ...
  postPassenger = async (tourdId) => {
    ...
    if (PassengerData.length > 0) {
      PassengerData.map(async (item, key) => {
        ...
        item.tourId = parseInt(tourdId);
        item.dob =
          item.dob != "" && item.dob != "1900-01-01" && item.dob != undefined
            ? formatDate(item.dob, "yyyy-MM-dd")
            : "1900-01-01T00:00:00";
        if (item.fName != "") {
          let postdata = {
            requestedUserId: parseInt(localStorage.getItem(USER_ID)),
          ...
          if (item.id == 0) {
            ...
            response = await SettingApi.PostSettingList(
              postdata,
              "/api/TourPassenger/Add"
            );
          } else {
            response = await SettingApi.PostSettingList(
              postdata,
              "/api/TourPassenger/Update"
            );
          }
          if (ArrayHelper.getValue(response, "isSuccess") == true) {
  ...
  getAgencyList = async () => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      this.setState({
        loader: false,
        agencyList: ArrayHelper.getValue(response, "agencies"),
      });
  ...
  getPassengerList = async (id) => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      this.setState({
        loader: false,
        PassengerList: ArrayHelper.getValue(response, "tourPassengers"),
      });
  ...
  itnerayServiceUpdate(item) {
    ...
    TourItineraryServiceData[0].serviceGTICommission =
      item.serviceGTICommission;
    TourItineraryServiceData[0].serviceGrossINR = item.serviceGrossINR;
    TourItineraryServiceData[0].serviceGSTPercentage =
    ...
    TourItineraryServiceData[0].serviceUSDMarkupPercentage =
      item.serviceUSDMarkupPercentage;
    TourItineraryServiceData[0].serviceUSDCommission =
      item.serviceUSDCommission;
    TourItineraryServiceData[0].serviceUSDClientDollar =
    ...
    TourItineraryServiceData[0].commissionType =
      item.markupPercentage == 0 ? "byAmount" : "byPercentage";
    TourItineraryServiceData[0].commissionTypeUS =
      item.serviceUSDMarkupPercentage == 0 ? "byAmount" : "byPercentage";
    TourItineraryServiceData[0].serviceUSDMarkUpAmount =
      item.serviceUSDCommission;
  ...
  submit = async (event: any) => {
    ...
    let error = "";
    if (
      ArrayHelper.getValue(this.state.toursData, "[0].leadType") == "Agency" &&
      ArrayHelper.getValue(this.state.toursData, "[0].agencyId") == ""
    ) {
      alert("Please select the Agency");
      error = "yes";
    } else if (
      ArrayHelper.getValue(this.state.toursData, "[0].leadType") == "Agency" &&
      ArrayHelper.getValue(this.state.toursData, "[0].agentId") == ""
    ) {
      alert("Please select the Agent");
      error = "yes";
    } else if (
      ArrayHelper.getValue(this.state.toursData, "[0].leadType") == "Direct" &&
      ArrayHelper.getValue(this.state.toursData, "[0].leadValue") == ""
    ) {
      ...
      ArrayHelper.getValue(this.state.toursData, "[0].tourTypeId") == ""
    ) {
      alert("Please select the Tour Type");
      error = "yes";
    } else if (
      ArrayHelper.getValue(this.state.toursData, "[0].statusFromGreavesId") ==
      ""
    ) {
      alert("Please select the Greaves Status");
      error = "yes";
    } else if (
      ArrayHelper.getValue(this.state.toursData, "[0].statusFromClientId") == ""
    ) {
      alert("Please select the Client Status");
      error = "yes";
    } else if (
      ArrayHelper.getValue(this.state.toursData, "[0].gtinConsultantId") == ""
    ) {
      alert("Please select the Operation Staff");
      error = "yes";
    } else if (
      ArrayHelper.getValue(this.state.toursData, "[0].gtusConsultantId") == ""
    ) {
      alert("Please select the Sales Staff");
      error = "yes";
    } else if (
      this.state.isImportItinerary == true &&
      ArrayHelper.getValue(
        this.state.TourItineraryData,
        "[0].tourItineraryService",
        []
    ...
    if (error == "") {
      ...
      let tourRecord = {
        ...
        agencyId:
          ArrayHelper.getValue(this.state.toursData, "[0].leadType") == "Agency"
            ...
            : 0,
        gtusConsultantId: parseInt(
          ArrayHelper.getValue(this.state.toursData, "[0].gtusConsultantId")
        ),
        gtinConsultantId: parseInt(
          ArrayHelper.getValue(this.state.toursData, "[0].gtinConsultantId")
        ),
        tourStartDate:
          ArrayHelper.getValue(this.state.toursData, "[0].tourStartDate") != ""
        ...
        statusFromGreavesId: parseInt(
          ArrayHelper.getValue(this.state.toursData, "[0].statusFromGreavesId")
        ),
        statusFromClientId: parseInt(
          ArrayHelper.getValue(this.state.toursData, "[0].statusFromClientId")
        ),
        noOfPax: parseInt(
          ArrayHelper.getValue(this.state.toursData, "[0].noOfPax")
        ),
        paymentLastDate: ArrayHelper.getValue(
          this.state.toursData,
          "[0].paymentLastDate"
          ...
          ? formatDate(
              ArrayHelper.getValue(this.state.toursData, "[0].paymentLastDate"),
              "yyyy-MM-dd"
            )
          : "1900-01-01T00:00:00",
        confirmationDate: ArrayHelper.getValue(
        ...
        salesRegionId:
          ArrayHelper.getValue(this.state.toursData, "[0].salesRegionId") != ""
            ? parseInt(
                ArrayHelper.getValue(this.state.toursData, "[0].salesRegionId")
              )
            : 0,
        ...
        tourContact: [
          {
            id: parseInt(
              ArrayHelper.getValue(this.state.toursData, "[0].traveler_id")
            ),
            tourId: parseInt(
              ArrayHelper.getValue(this.state.toursData, "[0].id")
            ),
            ...
            homePhone: ArrayHelper.getValue(
              this.state.toursData,
              "[0].traveler_home_phone"
            ),
            officePhone: ArrayHelper.getValue(
              this.state.toursData,
              "[0].traveler_office_phone"
            ),
            personalNo: ArrayHelper.getValue(
              this.state.toursData,
              "[0].traveler_cell"
            ),
            socialLink: ArrayHelper.getValue(
              this.state.toursData,
              "[0].traveler_socialLink"
            ),
          },
        ],
      };

      let hearedAboutList = this.state.hearedAboutList.filter(
        (_it) =>
          _it.name ==
          ArrayHelper.getValue(this.state.toursData, "[0].hearedAbout")
      );
      if (hearedAboutList.length == 0) {
        ...
        let responsehearedAbout = await SettingApi.PostSettingList(
          formDatahearedAbout,
          "/api/HearedAbout/Add"
        );
      }
      if (
        ...
        if (ArrayHelper.getValue(response, "isSuccess") == true) {
          ...
          this.setState({
            loader: false,
            name: "",
            message: "Tours  has been Updated",
            messageType: "success",
          });
          this.getToursList();
          setTimeout(() => {
        ...
        if (ArrayHelper.getValue(response, "isSuccess") == true) {
          ...
          this.postIntenary(
            ArrayHelper.getValue(response, "tourRecordResponse[0].id")
          );
          this.postPassenger(
            ArrayHelper.getValue(response, "tourRecordResponse[0].id")
          );
  ...
  reloadWindow = async () => {
    this.setState({ loader: false });
    let responseHearedAbount = await SettingApi.GetSettingList(
      "/api/HearedAbout/List"
    );
    if (ArrayHelper.getValue(responseHearedAbount, "isSuccess") == true) {
      this.setState({
        loader: false,
        hearedAboutList: ArrayHelper.getValue(
          responseHearedAbount,
          "hearedAbout"
        ),
      });
    } else {
    ...
    this.setState({ loader: false });
    let responseAppSetting = await SettingApi.GetSettingList(
      "/api/AppSetting/List"
    );
    if (ArrayHelper.getValue(responseAppSetting, "isSuccess") == true) {
      this.setState({
        loader: false,
        appSettingList: ArrayHelper.getValue(responseAppSetting, "appSettings"),
      });
      this.props.appSettingListInfo(
        ArrayHelper.getValue(responseAppSetting, "appSettings")
      );
      setTimeout(() => {
        this.addItinerarySerice("");
      }, 1000);
    ...
    if (ArrayHelper.getValue(responService, "isSuccess") == true) {
      this.setState({
        loader: false,
        serviceList: ArrayHelper.getValue(responService, "services"),
      });
      this.props.serviceListInfo(
        ArrayHelper.getValue(responService, "services")
      );
    } else {
      this.setState({ loader: false });
    }
    ...
    if (ArrayHelper.getValue(responseQuote, "isSuccess") == true) {
      this.setState({
        loader: false,
        quoteTypeList: ArrayHelper.getValue(responseQuote, "quoteTypes"),
      });
      this.props.quoteTypeListInfo(
        ArrayHelper.getValue(responseQuote, "quoteTypes")
      );
    } else {
      this.setState({ loader: false });
    }
    ...
    this.setState({ loader: true });
    let responseCountry = await SettingApi.GetSettingList(
      "/api/City/CountryList"
    );
    if (ArrayHelper.getValue(responseCountry, "isSuccess") == true) {
      ...
      countryList = "asc"
        ? orderBy(countryList, [(o: any) => parseInt(o.id)], ["asc"])
        : countryList;
    ...
    this.setState({ loader: true });
    let responseVendorType = await SettingApi.GetSettingList(
      "/api/VendorType/List"
    );
    if (ArrayHelper.getValue(responseVendorType, "isSuccess") == true) {
      let vendorTypeList = ArrayHelper.getValue(
        responseVendorType,
        "vendorTypes"
      );
      ...
      this.props.vendorTypeListInfo(
        ArrayHelper.getValue(responseVendorType, "vendorTypes")
      );
    } else {
      this.setState({ loader: false });
    }
    ...
    if (ArrayHelper.getValue(responseVendor, "isSuccess") == true) {
      this.setState({
        loader: false,
        vendorList: ArrayHelper.getValue(responseVendor, "vendors"),
      });
      this.props.vendorListInfo(
        ArrayHelper.getValue(responseVendor, "vendors")
      );
    } else {
      this.setState({ loader: false });
    }
    ...
    if (e != "") {
      ...
      if (ArrayHelper.getValue(response, "isSuccess") == true) {
        if (ArrayHelper.getValue(response, "tourItinerary").length > 0) {
          this.setState({
            loader: false,
            selectedItineraryList: ArrayHelper.getValue(
              response,
              "tourItinerary"
            ),
          });
  ...
  importItenery(e) {
    if (e != "") {
      ...
      let selectedItinerary = selectedItineraryList.filter(
        (item) => item.id == e
      )[0];
    ...
    return (
      <React.Fragment>
        ...
        <form method="post" onSubmit={this.submit}>
          <div
            className={`p-4 pt-0 ms-2 toursBlock   ${
              this.state.formDisply != true ? "hide" : ""
            }`}
          >
            <div
              className={`w-100 quot-sheet ${
                this.state.formDisply != true ? "hide" : ""
              }`}
            >
              <div className="row navbar navbar-expand-lg p-0 ">
                    ...
                    "" ? (
                      <li className="nav-item">
                        <a
                          ...
                          className={`nav-link ${
                            this.state.selectedTab == "Itinerary"
                              ? "active"
                              : ""
                          }`}
                        >
                    ...
                    {this.state.AllTourItineraryData.length > 0 ? (
                      <li className="nav-item dropdown">
                        <a
                          ...
                          className={`nav-link dropdown-toggle costing ${
                            this.state.selectedTab == "Quotation"
                              ? "active"
                              : ""
                          }`}
                        >
                ...
                <div className="col-md-4 d-sm-inline-flex align-items-center justify-content-end">
                  <div className="w-auto pt-2 me-2">
                    {" "}
            ...
            <TourItineraryFieldFormComponent
              ...
              addItinerarySerice={(e1, e2, e3) =>
                this.postItinerarySerice(e1, e2, e3)
              }
              deleteItinerarySerice={(e1, e2, e3) =>
                this.deleteItinerarySerice(e1, e2, e3)
              }
              ...
              fileUSDROE={ArrayHelper.getValue(
                this.state.toursData,
                "[0].fileUSDROE"
              )}
              ...
              noofGuest={ArrayHelper.getValue(
                this.state.toursData,
                "[0].noOfPax"
              )}
              isImportItinerary={this.state.isImportItinerary}
              setIsImportItinerary={(e) => {
                this.setState({
                  isImportItinerary: e,
                  selectedItineraryList: [],
                  selectedTourId: "",
            ...
            {this.state.AllTourItineraryData.length > 0 ? (
              <CostingViewComponent
                ...
                serviceList={this.state.serviceList}
                itnerayServiceUpdate={(e) => this.itnerayServiceUpdate(e)}
                selectedTab={this.state.selectedTab}
                TourItinerarySelected={this.state.TourItineraryData[0]}
                displayType="Edit"
                currency={ArrayHelper.getValue(
                  this.state.toursData,
                  "[0].currency"
            ...
            {this.state.AllTourItineraryData.length > 0 ? (
              <ClientMailLogViewComponent
                vendorList={this.state.vendorList}
                selectedTab={this.state.selectedTab}
                toursData={this.state.toursData[0]}
              />
            ) : (
              ""
            )}
            ...
            <div className="text-end p-4">
              ...
              "" ? (
                <button
                  className="btn btn-outlined rounded my-3 me-2"
                  type="button"
                >
                  Cancel
                </button>
              ...
              {ArrayHelper.getValue(this.props.toursSelectedData, "id") ==
              "" ? (
                <button className="btn rounded btn-primary my-3" type="submit">
                  {" "}
                  {ArrayHelper.getValue(this.props.toursSelectedData, "id") ==
                  ""
                    ? "Save"
        ...
        <CloneItineraryComponent
          getItinerary={() => this.getItinerary()}
          tourRecordId={ArrayHelper.getValue(
            this.props.toursSelectedData,
            "id"
          )}
          itineraryId={ArrayHelper.getValue(
            this.state.TourItineraryData,
            "[0].id"
          )}
        />
...
const mapDispatchToProps = (dispatch) => {
  return {
    toursListInfo: (data) =>
      dispatch({ type: actionTypesUser.TOURS_LIST, payload: data }),
    ...
    appSettingListInfo: (data) =>
      dispatch({ type: actionTypesUser.APP_SETTING_LIST, payload: data }),
  };
};
