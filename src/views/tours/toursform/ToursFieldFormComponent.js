...
class ToursFieldFormComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,tourNameListAll:[],toursData:[],agentFilterList:[],
            tourTypeList:[],greavesStatusList:[],clientStatusList:[],salesRegionList:[],userList:[],hearedAboutList:[],tourNameList:[]};
    }
        ...
        componentWillMount()
        {
            ...
            this.dataTypeList();
            this.setState({hearedAboutList:this.props.hearedAboutList,tourNameList:this.props.tourNameList})
        }
    
        dataTypeList=async()=>
        {
            let agentFilterList=this.props.agentList.filter((__item)=>__item.agencyId==ArrayHelper.getValue(this.props.toursData,'[0].agencyId')) 
                        setTimeout(()=>{
            ...
            this.props.toursListData.map((item:any)=>{
                ...
                tourNameList=tourNameList.concat([{id: item.tourName,name: item.tourName}]);
            ...
            if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
            ...
            this.setState({tourNameList:tourNameList,tourNameListAll:tourNameList})
    ...
    let salesRegionListData=this.props.salesRegionListData;
     ...
     else
     {
        ...
        let response= await SettingApi.GetSettingList('/api/User/List');
          if(ArrayHelper.getValue(response,'isSuccess')== true)
          {
            let userList=ArrayHelper.getValue(response,'userDetails').map((Item)=>{
        ...
        this.setState({
            toursData:  this.props.toursData.map((item: any, k: number) => {                
                if (index == k) {
                   if((key=='tourRecordId'))
                    {
                          item[key] = value.replace('--','-').replace(/[^A-Z0-9,-]/ig, '');
                    ...
                    else if((key=='currency'))
                    {
                        ...
                        let appSettingList=this.props.appSettingList.filter((_it)=>_it.settingName==settingName)
                        item['fileUSDROE'] = ArrayHelper.getValue(appSettingList,'[0].settingValue'); 
                    }
         ...
         handleOnSearch = (string, results) => {
            let tourNameList=this.state.tourNameListAll.filter((_it)=>_it.name.search(new RegExp(string.trim(), "i")) != -1); 
          ...
          handleOnSearchAboutGreaves = (string, results) => { 
            ...
            let hearedAboutList=this.props.hearedAboutList.filter((_it)=>_it.name.search(new RegExp(string.trim(), "i")) != -1); 
    ...
    {  // $("#tourStartDate").datepicker({minDate:"+0d"})
        ...
        .on("change", (e: any) => {
         $("#tourEndDate").datepicker("option", "minDate", addsDays(e.target.value,1));
         $("#serviveStartDate").datepicker("option", "minDate", addsDays(e.target.value,0));
         $("#serviceEndDate").datepicker("option", "minDate", addsDays(e.target.value,0));
         $("#flightDepartureDate").datepicker("option", "minDate", addsDays(e.target.value,0));
        ...
        .on("change", (e: any) => {
            $("#tourStartDate").datepicker("option", "maxDate", subsDays(e.target.value,1));
            $("#serviveStartDate").datepicker("option", "maxDate", subsDays(e.target.value,0));
            $("#serviceEndDate").datepicker("option", "maxDate", subsDays(e.target.value,0));
            $("#flightDepartureDate").datepicker("option", "maxDate", subsDays(e.target.value,0));
            $("#flightArrivalDate").datepicker("option", "maxDate", subsDays(e.target.value,0));
    ...
    render(){
        ...
        return(
            ...
            <React.Fragment>
                ...
                <div className={`borderless-box ${(this.props.selectedTab=='Tours' || this.props.selectedTab=='')?'show':'hide'}`}>
                <div className="maininfo">
                    <div className="d-flex mb-2">
                        ...
                        <div className="flex-fill p-1">
                            ...
                            <input maxLength={50} required  type="email"   value={ArrayHelper.getValue(this.props.toursData,'[0].traveler_email')}
                        ...
                        <div className="flex-fill p-1">
                            ...
                            <input maxLength={15} type="text" value={ArrayHelper.getValue(this.props.toursData,'[0].traveler_cell')}
                                             onChange={(event: any) => this.handleToursInput(0, 'traveler_cell', event.currentTarget.value)}
                                             
                                                 name="traveler_cell"  className="form-control form-control-sm"  placeholder="Enter Contact Number"/>
                        </div>
                        {/* <div className="flex-fill p-1">
            ...
            placeholder={ArrayHelper.getValue(this.props.toursData,'[0].hearedAbout')}
            autoFocus
          />
                    ...
                    <div className="d-flex  mb-2">
                        ...
                        <div className={`flex-fill p-1 ${(ArrayHelper.getValue(this.props.toursData,'[0].leadType')!='Agency')?'show':'hide'}`}>
                            <label  className="form-label">Lead Description</label>
                            <input maxLength={100}  type="text" className="form-control form-control-sm" name="leadValue" 
                        ...
                        <div className={`flex-fill p-1 ${(ArrayHelper.getValue(this.props.toursData,'[0].leadType')=='Agency')?'show':'hide'}`}>
                            ...
                            <DropdownList filter
                                  ...
                                  onChange={(event: any) => this.handleToursInput(0, 'agencyId', event.id)}
                                  defaultValue={ArrayHelper.getValue(this.props.toursData,'[0].agencyId')}
                                  dataKey='id'
                                  textField='name' />
                        ...
                        <div className={`flex-fill p-1 ${(ArrayHelper.getValue(this.props.toursData,'[0].leadType')=='Agency')?'show':'hide'}`}>
                            ...
                            <select  className="form-select form-select-sm" name="agentId" 
                                        ...
                                        {(this.props.agentList.length>0)?this.props.agentList.filter((__item)=>__item.agencyId==ArrayHelper.getValue(this.props.toursData,'[0].agencyId')).map((item,key)=>{
                        ...
                        <div className={`flex-fill p-1 ${(ArrayHelper.getValue(this.props.toursData,'[0].leadType')=='Agency' && ArrayHelper.getValue(this.props.toursData,'[0].agentId')!='')?'show':'hide'}`}>
                        ...
                        <div className="flex-fill p-1">
                            <label  className="form-label">Sales Region</label>
                                  ...
                                  onChange={(event: any) => this.handleToursInput(0, 'salesRegionId', event.id)}
                                  defaultValue={ArrayHelper.getValue(this.props.toursData,'[0].salesRegionId')}
                                  dataKey='id'
                                  textField='name' />
                        </div>
                        <div className="flex-fill p-1">
                            <label  className="form-label">Wetu Link {(ArrayHelper.getValue(this.props.toursData,'[0].wetuLink')!='')?<spna> -> <a target="_blank" href={ArrayHelper.getValue(this.props.toursData,'[0].wetuLink')}>View Link</a></spna>:''}</label>
                        ...
                        <label  className="form-label"></label><br/>
                                            <a data-bs-toggle="modal" data-bs-target="#agencyAdd" className="btn addInternary btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add  Agency</a>
                        </div>
                        <div className={`flex-fill p-1 ${(ArrayHelper.getValue(this.props.toursData,'[0].leadType')=='Agency')?'show':'hide'}`}>
                        <label  className="form-label"></label><br/>
                                 <a data-bs-toggle="modal" data-bs-target="#agentAdd" className="btn addInternaryService  btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add  Agent</a>
                        </div>
                        </div>
                        
                        <div className="d-flex  mb-2">
                        {(ArrayHelper.getValue(this.props.toursData,'[0].id')=='')?<div className="flex-fill p-1">
                            <label  className="form-label">Exitsig TR Number:</label>
                            <br/>
                            <input maxLength={50}   onChange={(event: any) => this.handleToursInput(0, 'exitingTr', ArrayHelper.getValue(this.props.toursData,'[0].exitingTr'))}   value={ArrayHelper.getValue(this.props.toursData,'[0].exitingTr')} type="checkbox"/>
                        </div>:''}
                        {(ArrayHelper.getValue(this.props.toursData,'[0].id')=='' && ArrayHelper.getValue(this.props.toursData,'[0].exitingTr')==true)?<div className="flex-fill p-1">
                            <label  className="form-label">Tour Id</label>
                            <input maxLength={50}  type="text" className="form-control form-control-sm" name="tourRecordId" 
                        ...
                        {(ArrayHelper.getValue(this.props.toursData,'[0].id')!='')?<div className="flex-fill p-1">
                            <label  className="form-label">Tour Id</label>
                            <br/>
                           {ArrayHelper.getValue(this.props.toursData,'[0].tourRecordId')}
                        </div>:''}
            ...
            showIcon={false}
            showClear={false}
            placeholder={ArrayHelper.getValue(this.props.toursData,'[0].tourName')}
            autoFocus
          />
                        ...
                        <div className="flex-fill p-1">
                            ...
                            {/* <br/> */}
                            <input readOnly id="tourStartDate" required type="text" placeholder="Enter Tour Start Date" className="form-control form-control-sm tourStartDate" name="tourStartDate" value={formatDate(ArrayHelper.getValue(this.props.toursData,'[0].tourStartDate'))}/>
                        ...
                        <div className="flex-fill p-1">
                            ...
                            <input readOnly id="tourEndDate" required type="text" placeholder="Enter Tour Start Date" className="form-control form-control-sm tourEndDate" name="tourEndDate" value={formatDate(ArrayHelper.getValue(this.props.toursData,'[0].tourEndDate'))}/>
                            {/* <br/> */}
                        ...
                        <div className="flex-fill p-1">
                            <label  className="form-label">Tour Type</label>
                                  ...
                                  onChange={(event: any) => this.handleToursInput(0, 'tourTypeId', event.id)}
                                  defaultValue={ArrayHelper.getValue(this.props.toursData,'[0].tourTypeId')}
                                  dataKey='id'
                                  textField='name' />
                        </div>
                        <div className="flex-fill p-1 dreavesStatus">
                            <label  className="form-label"> Greaves Status</label>
                                  ...
                                  onChange={(event: any) => this.handleToursInput(0, 'statusFromGreavesId', event.id)}
                                  defaultValue={ArrayHelper.getValue(this.props.toursData,'[0].statusFromGreavesId')}
                                  dataKey='id'
                                  textField='name' />
                        </div>
                        <div className="flex-fill p-1">
                            <label  className="form-label">Client Status</label>
                                  ...
                                  onChange={(event: any) => this.handleToursInput(0, 'statusFromClientId', event.id)}
                                  defaultValue={ArrayHelper.getValue(this.props.toursData,'[0].statusFromClientId')}
                                  dataKey='id'
                                  textField='name' />
                        </div>
                        <div className="flex-fill p-1">
                            <label  className="form-label">Operation Staff</label>
                                            ...
                                            <DropdownList filter
                                  data={this.state.userList.filter((item)=>item.departmentId=='2')}
                                  placeholder="Select Operation Staff"
                                  onChange={(event: any) => this.handleToursInput(0, 'gtinConsultantId', event.id)}
                                  defaultValue={ArrayHelper.getValue(this.props.toursData,'[0].gtinConsultantId')}
                                  dataKey='id'
                                  textField='fullName' />
                        </div>
                        <div className="flex-fill p-1">
                            <label  className="form-label">Sales Staff</label>
                                            ...
                                            <DropdownList filter
                                  data={this.state.userList.filter((item)=>item.departmentId=='1')}
                                  placeholder="Select Sales Staff"
                                  onChange={(event: any) => this.handleToursInput(0, 'gtusConsultantId', event.id)}
                                  defaultValue={ArrayHelper.getValue(this.props.toursData,'[0].gtusConsultantId')}
                                  dataKey='id'
                                  textField='fullName' />
                        </div>
                        
                    </div>
                    {(ArrayHelper.getValue(this.props.toursData,'[0].id')!='')? <div className="d-flex  mb-2">
                ...
                <PassengerFieldFormComponent
                  ...
                  traveler_name={ArrayHelper.getValue(this.props.toursData,'[0].traveler_name')}  
                  traveler_cell={ArrayHelper.getValue(this.props.toursData,'[0].traveler_cell')}
                  traveler_email={ArrayHelper.getValue(this.props.toursData,'[0].traveler_email')}            
             ...
             <FlightFieldFormComponent 
                  tourStartDate={ArrayHelper.getValue(this.props.toursData,'[0].tourStartDate')}
                  tourEndDate={ArrayHelper.getValue(this.props.toursData,'[0].tourEndDate')}             
                  ...
                  deleteVendorPayment={(e,e2) => this.props.deleteVendorPayment(e,e2)}
                  />
...
const mapStateToProps = state => {
    console.log(state.settingsData.salesRegionList,"state.settingsData.salesRegionList,state.settingsData.salesRegionList,");
