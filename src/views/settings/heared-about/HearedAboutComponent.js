...
class HearedAboutComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,hearedAboutListAll:[],hearedAboutListFilter:[],hearedAboutList:[],selectedHearedAbout:{},perPage:8,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(hearedAboutListData.length>0)
     {
        ...
        this.setState({hearedAboutListAll:hearedAboutListData,hearedAboutListFilter:hearedAboutListData,hearedAboutList:hearedAboutList});
     }
     else
     {
    ...
    gethearedAboutList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/HearedAbout/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'hearedAbout')[i],'id')!='') {
                hearedAboutList.push(ArrayHelper.getValue(response,'hearedAbout')[i]);
              }
          }
          
          this.setState({loader:false,hearedAboutListAll:ArrayHelper.getValue(response,'hearedAbout'),hearedAboutListFilter:ArrayHelper.getValue(response,'hearedAbout'),hearedAboutList:hearedAboutList});
          this.props.hearedAboutListInfo(ArrayHelper.getValue(response,'hearedAbout'));
        }
        else
        {
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.hearedAboutListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.hearedAboutListFilter[i],'id')!='') {
            hearedAboutList.push(this.state.hearedAboutListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), hearedAboutList: hearedAboutList });
          } else {
        ...
        updatedhearedAboutList=async(str)=>
        {
            ...
            hearedAboutListFilter=hearedAboutListFilter.map((item)=>{
              ...
              this.setState({hearedAboutListAll:hearedAboutListAll,hearedAboutListFilter:hearedAboutListFilter,hearedAboutList:hearedAboutList,selectedHearedAbout:str});
        ...
        HearedAboutDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Heared About?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/HearedAbout/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let hearedAboutListAll=this.state.hearedAboutListAll;
            hearedAboutListAll=hearedAboutListAll.filter((item)=>item.id!=str)
            let hearedAboutListFilter=this.state.hearedAboutListFilter;
            hearedAboutListFilter=hearedAboutListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,hearedAboutListAll:hearedAboutListAll,hearedAboutListFilter:hearedAboutListFilter});
              this.props.hearedAboutListInfo(hearedAboutListAll);
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({hearedAboutListFilter:hearedAboutListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/heared-about"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,hearedAboutListFilter:this.state.hearedAboutListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/heared-about"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = hearedAboutListFilter.length > 0 ? hearedAboutListFilter : hearedAboutListAll;
        return(
...
<div className="d-flex align-items-center justify-content-end">
                  ...
                  <button data-bs-toggle="modal" data-bs-target="#HearedAboutAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.hearedAboutList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.hearedAboutList.map((item,key)=>{
                                            return( <tr key={`HearedAbout-${key}`}>
                                                {/* <td>{key+1}</td> */}
                                                <td>{((this.state.currentPage -1)*this.state.perPage) + key+1}</td>

                                                <td>{ArrayHelper.getValue(item,'name')}</td>                                                
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#HearedAboutUpdate" onClick={()=>{this.setState({selectedHearedAbout:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                {/* <td>
                                ...
                                <p>No Record Found</p>}
                                {(this.state.hearedAboutListFilter.length > this.state.perPage) ?
                    <PaginationComponent
                ...
                <HearedAboutAddComponent gethearedAboutList={()=>this.gethearedAboutList()}/>
                <HearedAboutUpdateComponent updatedhearedAboutList={(str)=>this.updatedhearedAboutList(str)} selectedHearedAbout={this.state.selectedHearedAbout}/>
            </React.Fragment>
        )
    }
}  
...
const mapDispatchToProps = dispatch => {
    return {
        hearedAboutListInfo:(data)=>dispatch({type: HearedAboutsUser.HEARED_ABOUT_LIST,payload:data}),
       
    }
};

export default  connect(mapStateToProps, mapDispatchToProps)(HearedAboutComponent);