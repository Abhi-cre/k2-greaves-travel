...
class ContactChannelComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,contactChannelListAll:[],contactChannelListFilter:[],contactChannelList:[],selectedContactChannel:{},perPage:20,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(contactChannelListData.length>0)
     {
        ...
        this.setState({contactChannelListAll:contactChannelListData,contactChannelListFilter:contactChannelListData,contactChannelList:contactChannelList});
     }
     else
     {
    ...
    getContactChannelList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/ContactChannel/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'contactChannels')[i],'id')!='') {
                contactChannelList.push(ArrayHelper.getValue(response,'contactChannels')[i]);
              }
          }
          
          this.setState({loader:false,contactChannelListAll:ArrayHelper.getValue(response,'contactChannels'),contactChannelListFilter:ArrayHelper.getValue(response,'contactChannels'),contactChannelList:contactChannelList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.contactChannelListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.contactChannelListFilter[i],'id')!='') {
            contactChannelList.push(this.state.contactChannelListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), contactChannelList: contactChannelList });
          } else {
          ...
          if (ArrayHelper.getValue(this.state.contactChannelListFilter[i],'id')!='') {
            contactChannelList.push(this.state.contactChannelListFilter[i]);
              ...
              this.setState({ contactChannelList: contactChannelList, currentPage: 1 });
          }
        }
        ...
        updatedcontactChannelList=async(str)=>
        {
            ...
            contactChannelListFilter=contactChannelListFilter.map((item)=>{
              ...
              this.setState({contactChannelListAll:contactChannelListAll,contactChannelListFilter:contactChannelListFilter,contactChannelList:contactChannelList,selectedContactChannel:str});
        ...
        ContactStatusDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Contact Channel?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/ContactChannel/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let contactChannelListAll=this.state.contactChannelListAll;
            contactChannelListAll=contactChannelListAll.filter((item)=>item.id!=str)
            let contactChannelListFilter=this.state.contactChannelListFilter;
            contactChannelListFilter=contactChannelListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,contactChannelListAll:contactChannelListAll,contactChannelListFilter:contactChannelListFilter});
              this.props.contactChannelListInfo(contactChannelListAll);
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({contactChannelListFilter:contactChannelListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/contact-channel"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,contactChannelListFilter:this.state.contactChannelListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/contact-channel"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = contactChannelListFilter.length > 0 ? contactChannelListFilter : contactChannelListAll;
        return(
...
<div className="d-flex align-items-center justify-content-end">
                  ...
                  <button data-bs-toggle="modal" data-bs-target="#contactChannelAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.contactChannelList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.contactChannelList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#contactChannelUpdate" onClick={()=>{this.setState({selectedContactChannel:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                ...
                                <p>No Record Found</p>}
                                {(this.state.contactChannelListFilter.length > this.state.perPage) ?
                    <PaginationComponent
                ...
                <ContactChannelAddComponent getContactChannelList={()=>this.getContactChannelList()}/>
                <ContactChannelUpdateComponent updatedcontactChannelList={(str)=>this.updatedcontactChannelList(str)} selectedContactChannel={this.state.selectedContactChannel}/>
            </React.Fragment>
        )
    }
}  
...
export default  connect(mapStateToProps, mapDispatchToProps)(ContactChannelComponent);