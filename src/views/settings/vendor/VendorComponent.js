...
class VendorComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,vendorListAll:[],vendorListFilter:[],vendorList:[],selectedVendor:{},perPage:8,currentPage:1,vendorTypeList:[],vendorTypeId:'',vendorName:'',stateName:'',cityName:'' , countryName :''};
    }
    componentDidMount()
    {
     ...
     if(vendorListData.length>0)
     {
        ...
        this.setState({vendorListAll:vendorListData,vendorListFilter:vendorListData,vendorList:vendorList,vendorTypeList:this.props.vendorTypeListData});
     }
     else
     {
      this.getVendorList()
    ...
    getVendorList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/Vendor/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'vendors')[i],'id')!='') {
                vendorList.push(ArrayHelper.getValue(response,'vendors')[i]);
              }
          }
          
          this.setState({loader:false,vendorListFilter:ArrayHelper.getValue(response,'vendors'),vendorListAll:ArrayHelper.getValue(response,'vendors'),vendorList:vendorList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.vendorListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.vendorListFilter[i],'id')!='') {
            vendorList.push(this.state.vendorListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), vendorList: vendorList });
          } else {
        ...
        updatedVendorList=async(str)=>
        {
            ...
            vendorListFilter=vendorListFilter.map((item)=>{
              ...
              this.setState({vendorListAll:vendorListAll,vendorList:vendorList,vendorListFilter:vendorListFilter,selectedVendor:str});
              setTimeout(() => {
        ...
        AgencyDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Vendor?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/Vendor/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let vendorListAll=this.state.vendorListAll;
            ...
            vendorListFilter=vendorListFilter.filter((item)=>item.id!=str)
              ...
              this.setState({loader:false,vendorListAll:vendorListAll,vendorListFilter:vendorListFilter});
              setTimeout(() => {
        ...
        filterData()
        {
            ...
            let vendorListAll=this.state.vendorListAll;
                ...
                let vendorListFilter=  vendorListAll.map((item)=>{
                    ...
                    if(item.stateName.search(new RegExp(this.state.stateName.trim(), "i")) == -1 && this.state.stateName.trim()!='')
                    {
                        console.log('state')
                        item.display=false;
                    }
                    if(item.cityName.search(new RegExp(this.state.cityName.trim(), "i")) == -1 && this.state.cityName.trim()!='')
                    {
                        item.display=false;
                    }
                    if(item.countryName.search(new RegExp(this.state.countryName.trim() , 'i')) == -1 && this.state.countryName.trim()!=''){
                        console.log('hii')
                        item.display=false;
                    }
                    if(item.vendorTypeId !=this.state.vendorTypeId && this.state.vendorTypeId.trim()!='')
                    {
                        item.display=false;
                    }
              ...
              let   vendorListFilter1=vendorListFilter.filter((item)=>item.display==true)
      ...
      setTimeout(()=>{
        ...
        clearData()
        {
            this.setState({loader:true,'vendorName':'','vendorTypeId':'',stateName:'',cityName:'',currentPage:1,vendorListFilter:this.state.vendorListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/vendor"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = vendorListFilter.length > 0 ? vendorListFilter : vendorListAll;
        return(
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <br/>
                    <h5 className="">Vendorxx</h5>
                    </div>
                    <div className="col-md-8">
                        <div className="row g-3 align-items-center">
                            <div className="col-sm-2">
                                    <label>Name </label>
                                <input type="text" name="vendorName" placeholder="Name" value={this.state.vendorName} onChange={this.handleChange} className="form-control"  />
                            </div>
                            <div className="col-sm-2">
                            ...
                            <div className="col-sm-2">
                                ...
                                <input type="text" name="countryName" placeholder="Country Name" value={this.state.countryName} onChange={this.handleChange} className="form-control"  />
                            </div>
                            <div className="col-sm-2">
                                    <label>State Name </label>
                                <input type="text" name="stateName" placeholder="State Name" value={this.state.stateName} onChange={this.handleChange} className="form-control"  />
                            </div>
                            <div className="col-sm-2">
                                    <label>City Name </label>
                                <input type="text" name="cityName" placeholder="City Name" value={this.state.cityName} onChange={this.handleChange} className="form-control"  />
                            </div>
                    ...
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>   
                      <button data-bs-toggle="modal" data-bs-target="#vendorAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#vendorAdd" className="btn btn-outlined me-1">
...
<ExcelDownloadButton
  data={dataToExport}
  columns={['vendorName','vendorTypeName','countryName','stateName','cityName','email','primaryContactNo','address','zip']} 
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.vendorList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>                                          
                                            <th>Name</th>
                                    ...
                                    <tbody>
                                        {this.state.vendorList.map((item,key)=>{
                                            return( <tr key={`agency-${key}`}>
                                                ...
                                                <td>{ArrayHelper.getValue(item,'primaryContactNo')}</td> 
                                                <td>{ArrayHelper.getValue(item,'email')}</td>                                             
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#vendorView" onClick={()=>{this.setState({selectedVendor:item})}}  className="fa fa-eye btn"  data-bs-placement="top" title="Edit"></i>
                                                </td> 
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#vendorUpdate" onClick={()=>{this.setState({selectedVendor:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                              
                                                <td>
                ...
                <VendorUpdateComponent updatedVendorList={(str)=>this.updatedVendorList(str)} selectedVendor={this.state.selectedVendor}/>
