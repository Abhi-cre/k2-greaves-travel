...
class ClientStatusComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      loader: false,
      greavesStatusListAll: [],
      greavesStatusListFilter: [],
      greavesStatusList: [],
      selectedGreavesStatus: {},
      perPage: 8,
      currentPage: 1,
      keyword: "",
      filteredNames: [],
  ...
  componentDidMount() {
    ...
    if (greavesStatusListData.length > 0) {
      ...
      this.setState({
        greavesStatusListAll: greavesStatusListData,
        greavesStatusListFilter: greavesStatusListData,
        greavesStatusList: greavesStatusList,
      });
    } else {
  ...
  getGreavesStatusList = async () => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      ...
      for (let i = 0; i < this.state.perPage; i++) {
        if (
          ArrayHelper.getValue(
            ArrayHelper.getValue(response, "greavesStatuses")[i],
            "id"
          ) != ""
        ) {
          greavesStatusList.push(
            ArrayHelper.getValue(response, "greavesStatuses")[i]
          );
        }
      }
      ...
      this.setState({
        loader: false,
        greavesStatusListAll: ArrayHelper.getValue(response, "greavesStatuses"),
        greavesStatusListFilter: ArrayHelper.getValue(
          response,
          "greavesStatuses"
        ),
        greavesStatusList: greavesStatusList,
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.greavesStatusListFilter.length > 0) {
      ...
      if (
        ...
        for (i; i < numRecord; i++) {
          if (
            ArrayHelper.getValue(this.state.greavesStatusListFilter[i], "id") !=
            ""
          ) {
  ...
  updatedGreavesStatusList = async (str) => {
    ...
    this.setState({
      greavesStatusListAll: greavesStatusListAll,
      greavesStatusListFilter: greavesStatusListFilter,
      greavesStatusList: greavesStatusList,
      selectedGreavesStatus: str,
    });
  ...
  GreavesStatusDelete = async (str) => {
    if (window.confirm("Do you want to delete selected Greaves Status?")) {
      ...
      let response = await SettingApi.PostSettingList(
        formData,
        "/api/GreavesStatus/Delete"
      );
      if (ArrayHelper.getValue(response, "isSuccess") == true) {
        let greavesStatusListAll = this.state.greavesStatusListAll;
        greavesStatusListAll = greavesStatusListAll.filter(
          (item) => item.id != str
        );
        let greavesStatusListFilter = this.state.greavesStatusListAll;
        greavesStatusListFilter = greavesStatusListFilter.filter(
          (item) => item.id != str
        );
  ...
  clearData() {
    this.setState({
      ...
      keyword: "",
      currentPage: 1,
      greavesStatusListFilter: this.state.greavesStatusListAll,
    });
    setTimeout(() => {
      this.setState({ loader: false });
      this.props.history("/settings/greaves-status");
  ...
  handleChange = (e) => {
    ...
    if (name === "keyword") {
      const filteredNames = this.state.greavesStatusListFilter
        .map((user) => user.name)
  ...
  render() {
    ...
    const dataToExport =
      greavesStatusListFilter.length > 0
        ? greavesStatusListFilter
        : greavesStatusListAll;
    return (
      <React.Fragment>
          ...
          <div className="p-4">
              ...
              <div className="col-md-2 text-end">
                ...
                <div className="d-flex align-items-center justify-content-end">
                  ...
                  <button
                    ...
                    data-bs-target="#greavesStatusAdd"
                    className=""
                  ...
                  <ExcelDownloadButton
                    data={dataToExport}
                    columns={["name"]}
                    fileName="GreavesStatus.xlsx"
                    sheetName="GreavesStatus"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="borderless-box">
              <div className="table-responsive">
                {this.state.greavesStatusList.length > 0 ? (
                        ...
                        return (
                          <tr key={`actionType-${key}`}>
                            ...
                            <td>
                              <i
                                data-bs-toggle="modal"
                                data-bs-target="#greavesStatusUpdate"
                                onClick={() => {
                                  this.setState({
                                    selectedGreavesStatus: item,
                                  });
                                }}
                ...
                {this.state.greavesStatusListFilter.length >
                this.state.perPage ? (
                  <PaginationComponent
        ...
        <GreavesStatusAddComponent
          getGreavesStatusList={() => this.getGreavesStatusList()}
        />
        <GreavesStatusUpdateComponent
          updatedGreavesStatusList={(str) => this.updatedGreavesStatusList(str)}
          selectedGreavesStatus={this.state.selectedGreavesStatus}
        />
      </React.Fragment>
    );
  }
