...
class SalesRegionComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,salesRegionListAll:[],salesRegionListFilter:[],salesRegionList:[],selectedSalesRegion:{},perPage:8,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(salesRegionListData.length>0)
     {
        ...
        this.setState({salesRegionListAll:salesRegionListData,salesRegionListFilter:salesRegionListData,salesRegionList:salesRegionList});
     }
     else
     {
    ...
    getSalesRegionList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/SalesRegion/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'salesRegions')[i],'id')!='') {
                salesRegionList.push(ArrayHelper.getValue(response,'salesRegions')[i]);
              }
          }
          
          this.setState({loader:false,salesRegionListAll:ArrayHelper.getValue(response,'salesRegions'),salesRegionListFilter:ArrayHelper.getValue(response,'salesRegions'),salesRegionList:salesRegionList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.salesRegionListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.salesRegionListFilter[i],'id')!='') {
            salesRegionList.push(this.state.salesRegionListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), salesRegionList: salesRegionList });
          } else {
        ...
        updatedSalesRegionList=async(str)=>
        {
            ...
            salesRegionListFilter=salesRegionListFilter.map((item)=>{
              ...
              this.setState({salesRegionListAll:salesRegionListAll,salesRegionListFilter:salesRegionListFilter,salesRegionList:salesRegionList,selectedSalesRegion:str});
        ...
        SalesRegionDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Sales Region?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/SalesRegion/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let salesRegionListAll=this.state.salesRegionListAll;
            salesRegionListAll=salesRegionListAll.filter((item)=>item.id!=str)
            let salesRegionListFilter=this.state.salesRegionListFilter;
            salesRegionListFilter=salesRegionListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,salesRegionListAll:salesRegionListAll,salesRegionListFilter:salesRegionListFilter});
              this.props.salesRegionListInfo(salesRegionListAll);
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({salesRegionListFilter:salesRegionListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/sales-region"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,salesRegionListFilter:this.state.salesRegionListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/sales-region"); 
           },10)
        ...
        handleChange = (e) => {
            ...
            // downloadExcel = () => {
            //     const { salesRegionListAll } = this.state;
            
            //     if (salesRegionListAll.length === 0) {
            //         alert("No data available to export!");
            //         return;
            //     }
            
            //     // Prepare the data for the Excel file
            //     const formattedData = salesRegionListAll.map((city, index) => ({
            //         SrNo: index + 1,
            //         Name: city.name,
            ...
            //     XLSX.writeFile(wb, "Sales_Region.xlsx");
            // }

            // downloadExcel = () => {
            //     const { salesRegionListFilter, salesRegionListAll } = this.state;
            //     const dataToExport = salesRegionListFilter.length > 0 ? salesRegionListFilter : salesRegionListAll;
            ...
            //     XLSX.writeFile(wb, "SaleCategory.xlsx");
            // };
    render(){
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>   
                       <button data-bs-toggle="modal" data-bs-target="#salesRegionAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#salesRegionAdd" className="btn btn-outlined me-1">
                    ...
                    <i className="fa-sharp fa-solid fa-download"></i> Download Excel
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.salesRegionList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.salesRegionList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#salesRegionUpdate" onClick={()=>{this.setState({selectedSalesRegion:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                                    <i onClick={()=>this.SalesRegionDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                                ...
                                <p>No Record Found</p>}
                                {(this.state.salesRegionListFilter.length > this.state.perPage) ?
                    <PaginationComponent
                ...
                <SalesRegionAddComponent getSalesRegionList={()=>this.getSalesRegionList()}/>
                <SalesRegionUpdateComponent updatedSalesRegionList={(str)=>this.updatedSalesRegionList(str)} selectedSalesRegion={this.state.selectedSalesRegion}/>
            </React.Fragment>
        )
    }
}  
...
export default  connect(mapStateToProps, mapDispatchToProps)(SalesRegionComponent);