...
class QuoteTypeComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,quoteTypeListAll:[],quoteTypeListFilter:[],quoteTypeList:[],selectedQuoteType:{},perPage:20,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(quoteTypeListData.length>0)
     {
        ...
        this.setState({quoteTypeListAll:quoteTypeListData,quoteTypeListFilter:quoteTypeListData,quoteTypeList:quoteTypeList});
     }
     else
     {
    ...
    getQuoteTypeList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/QuoteType/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'quoteTypes')[i],'id')!='') {
                quoteTypeList.push(ArrayHelper.getValue(response,'quoteTypes')[i]);
              }
          }
          
          this.setState({loader:false,quoteTypeListAll:ArrayHelper.getValue(response,'quoteTypes'),quoteTypeListFilter:ArrayHelper.getValue(response,'quoteTypes'),quoteTypeList:quoteTypeList});
          this.props.quoteTypeListInfo(ArrayHelper.getValue(response,'quoteTypes'));
        }
        else
        {
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.quoteTypeListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.quoteTypeListFilter[i],'id')!='') {
            quoteTypeList.push(this.state.quoteTypeListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), quoteTypeList: quoteTypeList });
          } else {
        ...
        updatedQuoteTypeList=async(str)=>
        {
            ...
            quoteTypeListFilter=quoteTypeListFilter.map((item)=>{
              ...
              this.setState({quoteTypeListAll:quoteTypeListAll,quoteTypeListFilter:quoteTypeListFilter,quoteTypeList:quoteTypeList,selectedQuoteType:str});
        ...
        VendorTypeDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Quote Type?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/QuoteType/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let quoteTypeListAll=this.state.quoteTypeListAll;
            quoteTypeListAll=quoteTypeListAll.filter((item)=>item.id!=str)
            let quoteTypeListFilter=this.state.quoteTypeListFilter;
            quoteTypeListFilter=quoteTypeListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,quoteTypeListAll:quoteTypeListAll,quoteTypeListFilter:quoteTypeListFilter});
              this.props.quoteTypeListInfo(quoteTypeListAll);
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({quoteTypeListFilter:quoteTypeListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/quote-type"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,quoteTypeListFilter:this.state.quoteTypeListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/quote-type"); 
           },10)
        ...
        handleChange = (e) => {
            ...
            // downloadExcel = () => {
            //     const { quoteTypeListAll } = this.state;
            
            //     if (quoteTypeListAll.length === 0) {
            //         alert("No data available to export!");
            //         return;
            //     }
            
            //     // Prepare the data for the Excel file
            //     const formattedData = quoteTypeListAll.map((city, index) => ({
            //         SrNo: index + 1,
            //         Name: city.name,
            ...
            //     XLSX.writeFile(wb, "City_List.xlsx");
            // }


            // downloadExcel = () => {
            //     const { quoteTypeListFilter, quoteTypeListAll } = this.state;
            //     const dataToExport = quoteTypeListFilter.length > 0 ? quoteTypeListFilter : quoteTypeListAll;
    ...
    render(){
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <br/>
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>         <button data-bs-toggle="modal" data-bs-target="#vendorTypeAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.quoteTypeList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.quoteTypeList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#vendorTypeUpdate" onClick={()=>{this.setState({selectedQuoteType:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                ...
                <QuoteTypeAddComponent getQuoteTypeList={()=>this.getQuoteTypeList()}/>
                <QuoteTypeUpdateComponent updatedQuoteTypeList={(str)=>this.updatedQuoteTypeList(str)} selectedQuoteType={this.state.selectedQuoteType}/>
            </React.Fragment>
        )
    }
}  
