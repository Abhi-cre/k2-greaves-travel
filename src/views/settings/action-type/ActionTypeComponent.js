...
class ActionTypeComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,keyword:'',actionTypeListAll:[],actionTypeListFilter:[],actionTypeList:[],selectedActionType:{},perPage:8,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(actionTypeListData.length>0)
     {
        ...
        this.setState({actionTypeListAll:actionTypeListData,actionTypeListFilter:actionTypeListData,actionTypeList:actionTypeList});
    ...
    getActionTypeList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/ActionType/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'actionTypes')[i],'id')!='') {
                actionTypeList.push(ArrayHelper.getValue(response,'actionTypes')[i]);
              }
          }
          
          this.setState({loader:false,actionTypeListFilter:ArrayHelper.getValue(response,'actionTypes'),actionTypeListAll:ArrayHelper.getValue(response,'actionTypes'),actionTypeList:actionTypeList});
          this.props.actionTypeListInfo(ArrayHelper.getValue(response,'actionTypes'));
    ...
    componentWillReceiveProps(nextProps) {
        console.log('sssssssssssss',this.state.actionTypeListFilter.length,this.state.actionTypeListAll.length)
        if(this.state.actionTypeListFilter.length>0 )
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.actionTypeListFilter[i],'id')!='') {
            actionTypeList.push(this.state.actionTypeListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), actionTypeList: actionTypeList });
          } else {
        ...
        updatedActionTypeList=async(str)=>
        {
            ...
            actionTypeListFilter=actionTypeListFilter.map((item)=>{
              ...
              this.setState({actionTypeListFilter:actionTypeListFilter,actionTypeListAll:actionTypeListAll,actionTypeList:actionTypeList,selectedActionType:str});
        ...
        ActionTypeDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Action Type?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/ActionType/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let actionTypeListAll=this.state.actionTypeListAll;
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({actionTypeListFilter:actionTypeListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,actionTypeListFilter:this.state.actionTypeListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings"); 
           },10)
    ...
    render(){
        const { actionTypeListFilter, actionTypeListAll, currentPage, perPage } = this.state;
        const dataToExport = actionTypeListFilter.length > 0 ? actionTypeListFilter : actionTypeListAll;
        return(
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#actionTypeAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.actionTypeList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                    ...
                                    <tbody>
                                        {this.state.actionTypeList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#actionTypeUpdate" onClick={()=>{this.setState({selectedActionType:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                                    <i onClick={()=>this.ActionTypeDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                ...
                <ActionTypeAddComponent getActionTypeList={()=>this.getActionTypeList()}/>
                <ActionTypeUpdateComponent updatedActionTypeList={(str)=>this.updatedActionTypeList(str)} selectedActionType={this.state.selectedActionType}/>
            </React.Fragment>
        )
    }
}  
...
const mapDispatchToProps = dispatch => {
    return {
        actionTypeListInfo:(data)=>dispatch({type: actionTypesUser.ACTION_TYPE_LIST,payload:data}),
       
    }
};

export default  connect(mapStateToProps, mapDispatchToProps)(ActionTypeComponent);