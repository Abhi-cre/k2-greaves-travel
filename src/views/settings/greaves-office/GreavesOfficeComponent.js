...
class GreavesOfficeComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,greavesOfficeListAll:[],greavesOfficeListFilter:[],greavesOfficeList:[],selectedGreavesOffice:{},perPage:20,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(greavesOfficeListData.length>0)
     {
        ...
        this.setState({greavesOfficeListAll:greavesOfficeListData,greavesOfficeListFilter:greavesOfficeListData,greavesOfficeList:greavesOfficeList});
     }
     else
     {
    ...
    getGreavesOfficeList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/GreavesOffice/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'greavesOffices')[i],'id')!='') {
                greavesOfficeList.push(ArrayHelper.getValue(response,'greavesOffices')[i]);
              }
          }
          
          this.setState({loader:false,greavesOfficeListAll:ArrayHelper.getValue(response,'greavesOffices'),greavesOfficeListFilter:ArrayHelper.getValue(response,'greavesOffices'),greavesOfficeList:greavesOfficeList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.greavesOfficeListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.greavesOfficeListFilter[i],'id')!='') {
            greavesOfficeList.push(this.state.greavesOfficeListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), greavesOfficeList: greavesOfficeList });
          } else {
        ...
        updatedGreavesOfficeList=async(str)=>
        {
            ...
            greavesOfficeListFilter=greavesOfficeListFilter.map((item)=>{
              ...
              this.setState({greavesOfficeListAll:greavesOfficeListAll,greavesOfficeListFilter:greavesOfficeListFilter,greavesOfficeList:greavesOfficeList,selectedGreavesOffice:str});
        ...
        ClientStatusDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Greaves Office?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/GreavesOffice/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let greavesOfficeListAll=this.state.greavesOfficeListAll;
            greavesOfficeListAll=greavesOfficeListAll.filter((item)=>item.id!=str)
            let greavesOfficeListFilter=this.state.greavesOfficeListFilter;
            greavesOfficeListFilter=greavesOfficeListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,greavesOfficeListAll:greavesOfficeListAll,greavesOfficeListFilter:greavesOfficeListFilter});
              this.props.greavesOfficeListInfo(greavesOfficeListAll);
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({greavesOfficeListFilter:greavesOfficeListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/greaves-office"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,greavesOfficeListFilter:this.state.greavesOfficeListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/greaves-office"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = greavesOfficeListFilter.length > 0 ? greavesOfficeListFilter : greavesOfficeListAll;
        return(
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>  
                        <button data-bs-toggle="modal" data-bs-target="#greavesOfficeAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#greavesOfficeAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.greavesOfficeList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.greavesOfficeList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#clientStatusUpdate" onClick={()=>{this.setState({selectedGreavesOffice:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                                    <i onClick={()=>this.ClientStatusDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                                ...
                                <p>No Record Found</p>}
                                {(this.state.greavesOfficeListFilter.length > this.state.perPage) ?
                    <PaginationComponent
                ...
                <GreavesOfficeAddComponent getGreavesOfficeList={()=>this.getGreavesOfficeList()}/>
                <GreavesOfficeUpdateComponent updatedGreavesOfficeList={(str)=>this.updatedGreavesOfficeList(str)} selectedGreavesOffice={this.state.selectedGreavesOffice}/>
            </React.Fragment>
        )
    }
}  
...
export default  connect(mapStateToProps, mapDispatchToProps)(GreavesOfficeComponent);