...
class UserTypeComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,userTypeListAll:[],userTypeListFilter:[],userTypeList:[],selectedUserType:{},perPage:20,currentPage:1};
    }
    componentDidMount()
    {
     ...
     if(userTypeListData.length>0)
     {
        ...
        this.setState({userTypeListAll:userTypeListData,userTypeListFilter:userTypeListData,userTypeList:userTypeList});
     }
     else
     {
    ...
    getUserTypeList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/UserType/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'userTypes')[i],'id')!='') {
                userTypeList.push(ArrayHelper.getValue(response,'userTypes')[i]);
              }
          }
          
          this.setState({loader:false,userTypeListAll:ArrayHelper.getValue(response,'userTypes'),userTypeListFilter:ArrayHelper.getValue(response,'userTypes'),userTypeList:userTypeList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.userTypeListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.userTypeListFilter[i],'id')!='') {
            userTypeList.push(this.state.userTypeListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), userTypeList: userTypeList });
          } else {
        ...
        updatedUserTypeList=async(str)=>
        {
            ...
            userTypeListFilter=userTypeListFilter.map((item)=>{
              ...
              this.setState({userTypeListAll:userTypeListAll,userTypeListFilter:userTypeListFilter,userTypeList:userTypeList,selectedUserType:str});
        ...
        UserTypeDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected User Type?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/UserType/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let userTypeListAll=this.state.userTypeListAll;
            userTypeListAll=userTypeListAll.filter((item)=>item.id!=str)
            let userTypeListFilter=this.state.userTypeListAll;
            userTypeListFilter=userTypeListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,userTypeListAll:userTypeListAll,userTypeListFilter:userTypeListFilter});
              this.props.userTypeListInfo(userTypeListAll);
      ...
      setTimeout(()=>{
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,userTypeListFilter:this.state.userTypeListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/user-type"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = userTypeListFilter.length > 0 ? userTypeListFilter : userTypeListAll;
        return(
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>  
                       <button data-bs-toggle="modal" data-bs-target="#userTypeAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#userTypeAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.userTypeList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.userTypeList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#userTypeUpdate" onClick={()=>{this.setState({selectedUserType:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                                    <i onClick={()=>this.UserTypeDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                ...
                <UserTypeUpdateComponent updatedUserTypeList={(str)=>this.updatedUserTypeList(str)} selectedUserType={this.state.selectedUserType}/>
            </React.Fragment>
        )
    }
}  
