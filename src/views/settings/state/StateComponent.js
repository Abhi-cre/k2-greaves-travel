...
class StateComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,stateList:[],stateListAll:[],stateListFilter:[],countryList:[] ,selectedState:{},perPage:8,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(statelistData.length>0)
     {
        ...
        this.setState({stateListAll:statelistData,stateListFilter:statelistData,stateList:stateList});
     }
     else
     {
      this.getStateList()
    ...
    getStateList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/State/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'states')[i],'stateId')!='') {
                stateList.push(ArrayHelper.getValue(response,'states')[i]);
              }
          }
          this.setState({loader:false,stateListAll:ArrayHelper.getValue(response,'states'),stateListFilter:ArrayHelper.getValue(response,'states'),stateList:stateList});
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if(this.state.stateListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.stateListFilter[i],'stateId')!='') {
            stateList.push(this.state.stateListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), stateList: stateList });
          } else {
        ...
        updatedStateList=async(str)=>
        {
            ...
            stateListFilter=stateListFilter.map((item)=>{
              ...
              this.setState({stateListAll:stateListAll,stateListFilter:stateListFilter,stateList:stateList,selectedState:str});
              setTimeout(()=>{
        ...
        stateDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected State?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/State/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let stateListAll=this.state.stateListAll;
            ...
            stateListFilter=stateListFilter.filter((item)=>item.stateId!=str)
              ...
              this.setState({loader:false,stateListAll:stateListAll,stateListFilter:stateListFilter});
              setTimeout(() => {
      ...
      setTimeout(()=>{
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,stateListFilter:this.state.stateListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/state"); 
           },10)
    ...
    render(){
      ...
      const dataToExport = stateListFilter.length > 0 ? stateListFilter : stateListAll;
        return(
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.stateList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Country Name</th>
                                    ...
                                    <tbody>
                                        {this.state.stateList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#cityUpdate" onClick={()=>{this.setState({selectedState:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                                    <i onClick={()=>this.stateDelete(ArrayHelper.getValue(item,'stateId'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                ...
                <StateAddComponent getCountryList={()=>this.getCountryList()} getStateList={()=>this.getStateList()} stateList={this.state.stateList} countryList ={this.state.countryList}/>
                <StateUpdateComponent updatedStateList={(str)=>this.updatedStateList(str)} stateList={this.state.stateList} selectedState={this.state.selectedState} countryList ={this.state.countryList} />
            </React.Fragment>
        )
    }
}  
