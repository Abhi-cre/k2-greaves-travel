...
class AgencyTypeComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,agentTypeListAll:[],agentTypeListFilter:[],agentTypeList:[],selectedAgentType:{},perPage:20,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(agentTypeListData.length>0)
     {
        ...
        this.setState({agentTypeListAll:agentTypeListData,agentTypeListFilter:agentTypeListData,agentTypeList:agentTypeList});
     }
     else
     {
    ...
    getAgentTypeList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/AgentType/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'agentTypes')[i],'id')!='') {
                agentTypeList.push(ArrayHelper.getValue(response,'agentTypes')[i]);
              }
          }
          
          this.setState({loader:false,agentTypeListAll:ArrayHelper.getValue(response,'agentTypes'),agentTypeListFilter:ArrayHelper.getValue(response,'agentTypes'),agentTypeList:agentTypeList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.agentTypeListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.agentTypeListFilter[i],'id')!='') {
            agentTypeList.push(this.state.agentTypeListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), agentTypeList: agentTypeList });
          } else {
        ...
        updatedAgentTypeList=async(str)=>
        {
            ...
            agentTypeListFilter=agentTypeListFilter.map((item)=>{
              ...
              this.setState({agentTypeListAll:agentTypeListAll,agentTypeListFilter:agentTypeListFilter,agentTypeList:agentTypeList,selectedAgentType:str});
        ...
        AgentTypeDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Agent Type?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/AgentType/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let agentTypeListAll=this.state.agentTypeListAll;
            agentTypeListAll=agentTypeListAll.filter((item)=>item.id!=str)
            let agentTypeListFilter=this.state.agentTypeListAll;
            agentTypeListFilter=agentTypeListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,agentTypeListAll:agentTypeListAll,agentTypeListFilter:agentTypeListFilter});
              this.props.agentTypeListInfo(agentTypeListAll);
              this.clearData();  
                }
            }   
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({agentTypeListFilter:agentTypeListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,agentTypeListFilter:this.state.agentTypeListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/agent-type"); 
           },10)
        ...
        handleChange = (e) => {
            ...
            // downloadExcel = () => {
            //     const { agentTypeListAll } = this.state;
            
            //     if (agentTypeListAll.length === 0) {
            //         alert("No data available to export!");
            //         return;
            //     }
            //     const formattedData = agentTypeListAll.map((action, index) => ({
            //         SrNo: index + 1,
            //         Name: action.name,
            ...
            //     XLSX.writeFile(wb, "AgentType_List.xlsx");
            // }
    render(){
        const { agentTypeListFilter, agentTypeListAll } = this.state;
        const dataToExport = agentTypeListFilter.length > 0 ? agentTypeListFilter : agentTypeListAll;
        return(
...
<div className="d-flex align-items-center justify-content-end">
                  ...
                  <button data-bs-toggle="modal" data-bs-target="#agentTypeAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.agentTypeList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>                                           
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.agentTypeList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#agentTypeUpdate" onClick={()=>{this.setState({selectedAgentType:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                                    <i onClick={()=>this.AgentTypeDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                ...
                <AgentTypeAddComponent getAgentTypeList={()=>this.getAgentTypeList()}/>
                <AgentTypeUpdateComponent updatedAgentTypeList={(str)=>this.updatedAgentTypeList(str)} selectedAgentType={this.state.selectedAgentType}/>
            </React.Fragment>
        )
    }
}  
...
export default  connect(mapStateToProps, mapDispatchToProps)(AgencyTypeComponent);