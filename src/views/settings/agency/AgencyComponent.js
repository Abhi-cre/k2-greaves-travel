...
class AgencyComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,agencyListAll:[],agencyListFilter:[],agencyList:[],selectedAgency:{},perPage:8,currentPage:1,'name':'','agnecyTypeName':'','greavesOfficeName':'','salesRegionName':''};
    }
    componentDidMount()
    {
     ...
     if(agencyListData.length>0)
     {
        ...
        this.setState({agencyListAll:agencyListData,agencyListFilter:agencyListData,agencyList:agencyList});
     }
     else
     {
      this.getAgencyList()
    ...
    getAgencyList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/Agency/List');
        ...
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'agencies')[i],'id')!='') {
                agencyList.push(ArrayHelper.getValue(response,'agencies')[i]);
              }
          }
          
          this.setState({loader:false,agencyListAll:ArrayHelper.getValue(response,'agencies'),agencyListFilter:ArrayHelper.getValue(response,'agencies'),agencyList:agencyList});
          setTimeout(() => {
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.agencyListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.agencyListFilter[i],'id')!='') {
            agencyList.push(this.state.agencyListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), agencyList: agencyList });
          } else {
        ...
        updatedAgencyList=async(str)=>
        {
            ...
            agencyListFilter=agencyListFilter.map((item)=>{
              ...
              this.setState({agencyListAll:agencyListAll,agencyListFilter:agencyListFilter,agencyList:agencyList,selectedAgency:str});
              setTimeout(() => {
        ...
        AgencyDelete=async(str)=>
        {
            ...
            if(window.confirm('Do you want to delete selected Agency?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/Agency/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let agencyListAll=this.state.agencyListAll;
            ...
            agencyListFilter=agencyListFilter.filter((item)=>item.id!=str)
              ...
              this.setState({loader:false,agencyListAll:agencyListAll,agencyListFilter:agencyListFilter});
              setTimeout(() => {
        ...
        filterData()
        {
            ...
            let agencyListAll=this.state.agencyListAll;
                ...
                let agencyListFilter=  agencyListAll.map((item)=>{
                    ...
                    if(item.agnecyTypeName.search(new RegExp(this.state.agnecyTypeName.trim(), "i")) == -1 && this.state.agnecyTypeName.trim()!='')
                    {
                        item.display=false;
                    }
                    if(item.greavesOfficeName.search(new RegExp(this.state.greavesOfficeName.trim(), "i")) == -1 && this.state.greavesOfficeName.trim()!='')
                    {
                        item.display=false;
                    }
                    if(item.salesRegionName.search(new RegExp(this.state.salesRegionName.trim(), "i")) == -1 && this.state.salesRegionName.trim()!='')
                    {
                        item.display=false;
                    }
                    return item;
                ...
                let   agencyListFilter1=agencyListFilter.filter((item)=>item.display==true)
             
                this.setState({agencyListFilter:agencyListFilter1,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/agency"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,name:'',agnecyTypeName:'',greavesOfficeName:'',salesRegionName:'',currentPage:1,agencyListFilter:this.state.agencyListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/agency"); 
           },10)
        ...
        handleChange = (e) => {
            ...
            //     const dataToExport = agencyListFilter.length > 0 ? agencyListFilter : agencyListAll;
            ...
            //         SrNo: index + 1,
            //         Name: city.name,
            //         Agency_Typetate : city.agnecyTypeName,
            //         Greaves_Office: city.greavesOfficeName,
            //         Sales_Region: city.salesRegionName,
            //         Email:city.email,
            //         Zip:city.zip,
            ...
            //     XLSX.writeFile(wb, "Airline.xlsx");
            // };
    ...
    render(){
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <div className="col-md-8">
                        <div className="row g-3 align-items-center">
                            ...
                            <div className="col-sm-2">
                                ...
                                <input type="text" name="agnecyTypeName" value={this.state.agnecyTypeName} onChange={this.handleChange} className="form-control"  placeholder="Agency Type"/>
                            </div>
                            <div className="col-sm-2">
                                    <label>Greaves Office </label>
                                <input type="text" name="greavesOfficeName" value={this.state.greavesOfficeName} onChange={this.handleChange} className="form-control"  placeholder="Greaves Office"/>
                            </div>
                            <div className="col-sm-2">
                                    <label>Sales Region </label>
                                <input type="text" name="salesRegionName" value={this.state.salesRegionName} onChange={this.handleChange} className="form-control"  placeholder="Sales Region"/>
                            </div>
                    ...
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>     
                      <button data-bs-toggle="modal" data-bs-target="#agencyAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#agencyAdd" className="btn btn-outlined me-1">
                  ...
                  {/* <button onClick={this.downloadExcel} type="button" className="btn btn-outlined ms-1">
                    <i className="fa-sharp fa-solid fa-download"></i> Download Excel
                                                      ...
                                                      <ExcelDownloadButton
  data={dataToExport}
  columns={['name','agnecyTypeName','greavesOfficeName','salesRegionName','email','zip','address','cityName','stateName','countryName']} 
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.agencyList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>                                          
                                            <th>Name</th>
                                    ...
                                    <tbody>
                                        {this.state.agencyList.map((item,key)=>{
                                            return( <tr key={`agency-${key}`}>
                                                {/* <td>{key+1}</td> */}
                                                <td>{((this.state.currentPage -1)*this.state.perPage) + key+1}</td>

                                               
                                                <td>{ArrayHelper.getValue(item,'name')}</td>   
                                                <td>{ArrayHelper.getValue(item,'agnecyTypeName')}</td>  
                                                <td>{ArrayHelper.getValue(item,'greavesOfficeName')}</td> 
                                                <td>{ArrayHelper.getValue(item,'salesRegionName')}</td>                                             
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#agencyView" onClick={()=>{this.setState({selectedAgency:item})}}  className="fa fa-eye btn"  data-bs-placement="top" title="Edit"></i>
                                                </td> 
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#agencyUpdate" onClick={()=>{this.setState({selectedAgency:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                              
                                                <td>
                                                    <i onClick={()=>this.AgencyDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                ...
                <AgencyUpdateComponent updatedAgencyList={(str)=>this.updatedAgencyList(str)} selectedAgency={this.state.selectedAgency}/>
