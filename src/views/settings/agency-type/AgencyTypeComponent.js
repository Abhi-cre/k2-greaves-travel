...
class AgencyTypeComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,agencyTypeListAll:[],agencyTypeListFilter:[],agencyTypeList:[],selectedAgencyType:{},perPage:8,currentPage:1,keyword:'',agencyTypeListFilter:[],agencyTypeListAll: []};
    }
    componentDidMount()
    {
     ...
     if(agencyTypeListData.length>0)
     {
        ...
        this.setState({agencyTypeListAll:agencyTypeListData,agencyTypeListFilter:agencyTypeListData,agencyTypeList:agencyTypeList});
     }
     else
     {
    ...
    getagencyTypeList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/AgencyType/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'agencyTypes')[i],'id')!='') {
                agencyTypeList.push(ArrayHelper.getValue(response,'agencyTypes')[i]);
              }
          }
          
          this.setState({loader:false,agencyTypeListAll:ArrayHelper.getValue(response,'agencyTypes'),agencyTypeListFilter:ArrayHelper.getValue(response,'agencyTypes'),agencyTypeList:agencyTypeList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.agencyTypeListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.agencyTypeListFilter[i],'id')!='') {
            agencyTypeList.push(this.state.agencyTypeListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), agencyTypeList: agencyTypeList });
          } else {
        ...
        updatedAgencyTypeList=async(str)=>
        {
            ...
            agencyTypeList=agencyTypeList.map((item)=>{
              ...
              this.setState({agencyTypeListAll:agencyTypeListAll,agencyTypeList:agencyTypeList,selectedAgencyType:str});
        ...
        ActionTypeDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Agency Type?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/AgencyType/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let agencyTypeListAll=this.state.agencyTypeListAll;
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({agencyTypeListFilter:agencyTypeListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,agencyTypeListFilter:this.state.agencyTypeListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/agency-type"); 
           },10)
    ...
    render(){
        const { agencyTypeListFilter, agencyTypeListAll, currentPage, perPage } = this.state;
        const dataToExport = agencyTypeListFilter.length > 0 ? agencyTypeListFilter : agencyTypeListAll;
        return(
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <br/>
                    <h5 className="">Agency Typesss</h5>
                    </div>
                ...
                <div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()} style={{ marginRight: '30px' }}>
                    <img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#agencyTypeAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.agencyTypeList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                    ...
                                    <tbody>
                                        {this.state.agencyTypeList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#agencyTypeUpdate" onClick={()=>{this.setState({selectedAgencyType:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                ...
                <AgencyTypeAddComponent getagencyTypeList={()=>this.getagencyTypeList()}/>
                <AgencyTypeUpdateComponent updatedAgencyTypeList={(str)=>this.updatedAgencyTypeList(str)} selectedAgencyType={this.state.selectedAgencyType}/>
            </React.Fragment>
        )
    }
}  
