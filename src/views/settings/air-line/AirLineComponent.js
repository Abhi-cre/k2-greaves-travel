...
class AirLineComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    if (airLineListData.length > 0) {
      ...
      this.setState({
        airLineListAll: airLineListData,
        airLineListFilter: airLineListData,
  ...
  getAirLineList = async () => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      ...
      for (let i = 0; i < this.state.perPage; i++) {
        if (
          ArrayHelper.getValue(
            ArrayHelper.getValue(response, "airLines")[i],
            "id"
          ) != ""
        ) {
      ...
      this.setState({
        loader: false,
        airLineListAll: ArrayHelper.getValue(response, "airLines"),
        airLineListFilter: ArrayHelper.getValue(response, "airLines"),
        airLineList: airLineList,
      });
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.airLineListFilter.length > 0) {
      ...
      if (
        ...
        for (i; i < numRecord; i++) {
          if (
            ArrayHelper.getValue(this.state.airLineListFilter[i], "id") != ""
          ) {
  ...
  updatedAirLineList = async (str) => {
    ...
    this.setState({
      airLineListAll: airLineListAll,
      airLineListFilter: airLineListFilter,
      airLineList: airLineList,
      selectedAirLine: str,
    });
  ...
  filterData() {
    ...
    let airLineListFilter = airLineListAll.map((item) => {
      ...
      if (
        item.code.search(new RegExp(this.state.code.trim(), "i")) == -1 &&
    ...
    let airLineListFilter1 = airLineListFilter.filter(
  ...
  clearData() {
    this.setState({
      ...
      code: "",
      currentPage: 1,
      airLineListFilter: this.state.airLineListAll,
    });
    setTimeout(() => {
      this.setState({ loader: false });
      this.props.history("/settings/air-line");
  ...
  render() {
    ...
    const dataToExport =
      airLineListFilter.length > 0 ? airLineListFilter : airLineListAll;
    return (
      <React.Fragment>
          ...
          <div className="p-4">
            ...
            <div className="borderless-box">
              <div className="table-responsive">
                {this.state.airLineList.length > 0 ? (
                        ...
                        return (
                          <tr key={`actionType-${key}`}>
                            ...
                            <td>
                              <i
                                data-bs-toggle="modal"
                                data-bs-target="#airLineUpdate"
                                onClick={() => {
                                  this.setState({ selectedAirLine: item });
                                }}
        ...
        <AirLineUpdateComponent
          updatedAirLineList={(str) => this.updatedAirLineList(str)}
...
const mapDispatchToProps = (dispatch) => {
  return {
    airLineListInfo: (data) =>
      dispatch({ type: actionTypesUser.AIR_LINE_LIST, payload: data }),
  };
};
