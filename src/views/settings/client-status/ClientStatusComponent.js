...
class ClientStatusComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,clientStatusListAll:[],clientStatusListFilter:[],clientStatusList:[],selectedClientStatus:{},perPage:8,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(clientStatusListData.length>0)
     {
        ...
        this.setState({clientStatusListAll:clientStatusListData,clientStatusListFilter:clientStatusListData,clientStatusList:clientStatusList});
     }
     else
     {
    ...
    getClientStatusList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/ClientStatus/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'clientStatuses')[i],'id')!='') {
                clientStatusList.push(ArrayHelper.getValue(response,'clientStatuses')[i]);
              }
          }
          
          this.setState({loader:false,clientStatusListAll:ArrayHelper.getValue(response,'clientStatuses'),clientStatusListFilter:ArrayHelper.getValue(response,'clientStatuses'),clientStatusList:clientStatusList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.clientStatusListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.clientStatusListFilter[i],'id')!='') {
            clientStatusList.push(this.state.clientStatusListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), clientStatusList: clientStatusList });
          } else {
        ...
        updatedclientStatusList=async(str)=>
        {
            ...
            clientStatusListFilter=clientStatusListFilter.map((item)=>{
              ...
              this.setState({clientStatusListAll:clientStatusListAll,clientStatusListFilter:clientStatusListFilter,clientStatusList:clientStatusList,selectedClientStatus:str});
        ...
        ClientStatusDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Client Status?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/ClientStatus/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let clientStatusListAll=this.state.clientStatusListAll;
            clientStatusListAll=clientStatusListAll.filter((item)=>item.id!=str)
            let clientStatusListFilter=this.state.clientStatusListFilter;
            clientStatusListFilter=clientStatusListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,clientStatusListAll:clientStatusListAll,clientStatusListFilter:clientStatusListFilter});
              this.props.clientStatusListInfo(clientStatusListAll);
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({clientStatusListFilter:clientStatusListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/client-status"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,clientStatusListFilter:this.state.clientStatusListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/client-status"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = clientStatusListFilter.length > 0 ? clientStatusListFilter : clientStatusListAll;
        return(
...
<div className="d-flex align-items-center justify-content-end">
                  ...
                  <button data-bs-toggle="modal" data-bs-target="#clientStatusAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.clientStatusList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.clientStatusList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#clientStatusUpdate" onClick={()=>{this.setState({selectedClientStatus:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                ...
                                <p>No Record Found</p>}
                                {(this.state.clientStatusListFilter.length > this.state.perPage) ?
                    <PaginationComponent
                ...
                <ClientStatusAddComponent getClientStatusList={()=>this.getClientStatusList()}/>
                <ClientStatusUpdateComponent updatedclientStatusList={(str)=>this.updatedclientStatusList(str)} selectedClientStatus={this.state.selectedClientStatus}/>
            </React.Fragment>
        )
    }
}  
