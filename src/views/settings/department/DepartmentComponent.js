...
class DepartmentComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    if (departmentListData.length > 0) {
      ...
      this.setState({
        departmentListAll: departmentListData,
        departmentListFilter: departmentListData,
  ...
  getDepartmentList = async () => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      ...
      for (let i = 0; i < this.state.perPage; i++) {
        if (
          ArrayHelper.getValue(
            ArrayHelper.getValue(response, "departments")[i],
            "id"
          ) != ""
        ) {
      ...
      this.setState({
        loader: false,
        departmentListAll: ArrayHelper.getValue(response, "departments"),
        departmentListFilter: ArrayHelper.getValue(response, "departments"),
        departmentList: departmentList,
      });
      this.props.departmentListInfo(
        ArrayHelper.getValue(response, "departments")
      );
    } else {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.departmentListFilter.length > 0) {
      ...
      if (
        ...
        for (i; i < numRecord; i++) {
          if (
            ArrayHelper.getValue(this.state.departmentListFilter[i], "id") != ""
          ) {
  ...
  ContactStatusDelete = async (str) => {
    if (window.confirm("Do you want to delete selected Department?")) {
      ...
      if (ArrayHelper.getValue(response, "isSuccess") == true) {
        ...
        departmentListFilter = departmentListFilter.filter(
          (item) => item.id != str
        );
  ...
  clearData() {
    this.setState({
      ...
      keyword: "",
      currentPage: 1,
      departmentListFilter: this.state.departmentListAll,
    });
    setTimeout(() => {
      this.setState({ loader: false });
      this.props.history("/settings/department");
  ...
  handleChange = (e) => {
    ...
    if (name === "keyword") {
      const filteredNames = this.state.departmentListFilter
        .map((user) => user.name)
    ...
    return (
      <React.Fragment>
                        ...
                        return (
                          <tr key={`actionType-${key}`}>
                            ...
                            <td>
                              <i
                                data-bs-toggle="modal"
                                data-bs-target="#departmentUpdate"
                                onClick={() => {
        ...
        <DepartmentUpdateComponent
          updatedDepartmentList={(str) => this.updatedDepartmentList(str)}
