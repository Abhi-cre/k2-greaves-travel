...
class ServiceComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,vendorTypeList:[],vendorList:[],serviceListFilter:[],serviceListAll:[],serviceList:[],selectedService:{},perPage:8,currentPage:1,'vendorTypeId':0,'name':'','vendorType':'','vendorName':'','startDate':'','endDate':''};
    }
    componentDidMount()
    {
        $("#searchStartDate").datepicker()
        .on("change", (e: any) => {
         $("#searchEndDate").datepicker("option", "minDate", addsDays(e.target.value,1));
        ...
        .on("change", (e: any) => {
            $("#searchStartDate").datepicker("option", "maxDate", subsDays(e.target.value,1));
     ...
     if(serviceListData.length>0)
     {
        ...
        this.setState({serviceListAll:serviceListData,serviceList:serviceList,vendorTypeList:this.props.vendorTypeListData});
     }
     else
     {
      this.getServiceList()
    ...
    getServiceList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/Service/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'services')[i],'id')!='') {
                serviceList.push(ArrayHelper.getValue(response,'services')[i]);
              }
          }
          
          this.setState({loader:false,serviceListFilter:ArrayHelper.getValue(response,'services'),serviceListAll:ArrayHelper.getValue(response,'services'),serviceList:serviceList});
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if(this.state.serviceListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.serviceListFilter[i],'id')!='') {
            serviceList.push(this.state.serviceListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), serviceList: serviceList });
          } else {
        ...
        updatedServiceList=async(str)=>
        {
            ...
            serviceList=serviceList.map((item)=>{
              ...
              this.setState({serviceListAll:serviceListAll,serviceList:serviceList,selectedService:str});
        ...
        ServiceDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Service?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/Service/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let serviceListAll=this.state.serviceListAll;
            ...
            serviceList=serviceList.filter((item)=>item.id!=str)
              ...
              this.setState({loader:false,serviceListAll:serviceListAll,serviceList:serviceList,serviceListFilter:serviceListFilter});
              //this.props.serviceListInfo(serviceListAll);
        ...
        filterData()
        {
            ...
            let serviceListAll=this.state.serviceListAll;
                ...
                let serviceListFilter=  serviceListAll.map((item)=>{
                    ...
                    if(item.vendorTypeId !=this.state.vendorTypeId && this.state.vendorTypeId!='')
                    {
                        item.display=false;
                    }
                    if(item.vendorName.search(new RegExp(this.state.vendorName.trim(), "i")) == -1 && this.state.vendorName.trim()!='')
                    {
                        item.display=false;
                    }
                    if(GETDATETIME(item.startDate) <  GETDATETIME(this.state.startDate) && this.state.startDate.trim()!='')
                    {
                        item.display=false;
                    }
                    if(GETDATETIME(item.endDate) >  GETDATETIME(this.state.endDate) && this.state.endDate.trim()!='')
                    {
                        item.display=false;
                    }
                    return item;
              ...
              let   serviceListFilter1=serviceListFilter.filter((item)=>item.display==true);
      ...
      setTimeout(()=>{
        ...
        clearData()
        {
            this.setState({loader:true,'name':'','vendorTypeId':'','vendorName':'','startDate':'','endDate':'',currentPage:1,serviceListFilter:this.state.serviceListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/service"); 
           },10)
        ...
        handleChange = (e) => {
            ...
            downloadExcel = () => {
                ...
                const formattedData = dataToExport.map((item, index) => {
                    const vendorType = vendorTypeList.find(vt => vt.id === item.vendorTypeId);
                    const vendorTypeName = vendorType ? vendorType.name : ''; 
            
                    const startDate = ArrayHelper.getValue(item, 'serviceFeeDetails[0].startDate');
                    const endDate = ArrayHelper.getValue(item, 'serviceFeeDetails[0].endDate');
            
                    const formattedStartDate = startDate && startDate !== '0001-01-01T00:00:00' ? formatDate(startDate, 'dd-MM-yyyy') : '';
                    const formattedEndDate = endDate && endDate !== '0001-01-01T00:00:00' ? formatDate(endDate, 'dd-MM-yyyy') : '';
                    ...
                    Object.keys(formattedRow).forEach((key) => {
                        if (!columns.includes(key) && formattedRow[key] !== undefined && formattedRow[key] !== null && formattedRow[key] !== '') {
                            columns.push(key);
                        }
                    });
                ...
                const finalData = formattedData.filter((row) => 
                    columns.some((column) => row[column] !== undefined && row[column] !== null && row[column] !== '')
                );
                ...
                const columnWidths = columns.map((column) => {
                    ...
                    return { wpx: Math.max(maxLength * 8, 100) }; // Ensure wider columns for large data
                });
    ...
    render(){
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <div className="col-md-8">
                        <div className="row g-3 align-items-center">
                            <div className="col-sm-2">
                                    <label>Name </label>
                                <input type="text" name="name" placeholder="Name" value={this.state.name} onChange={this.handleChange} className="form-control"  />
                            </div>
                            <div className="col-sm-2">
                                    <label>Vendor Type </label>
                                    <select required className="form-select form-noradious" name="vendorTypeId" value={this.state.vendorTypeId} onChange={this.handleChange} onChange={this.handleChange} >
                            ...
                            <div className="col-sm-2">
                                ...
                                <input type="text" name="vendorName" placeholder="Vendor Name" value={this.state.vendorName} onChange={this.handleChange} className="form-control"/>
                            </div>
                            {/* <div className="col-sm-2">
                    ...
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>    
                      <button data-bs-toggle="modal" data-bs-target="#serviceAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#serviceAdd" className="btn btn-outlined me-1">
                    ...
                    <i className="fa-sharp fa-solid fa-download"></i> Download Excel
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.serviceList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>                                          
                                            <th>Name</th>
                                    ...
                                    <tbody>
                                        {this.state.serviceList.map((item,key)=>{
                                            return( <tr key={`service-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#agencyView" onClick={()=>{this.setState({selectedService:item})}}  className="fa fa-eye btn"  data-bs-placement="top" title="Edit"></i>
                                                </td> 
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#serviceUpdate" onClick={()=>{this.setState({selectedService:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                              
                                                <td>
                                                    <i onClick={()=>this.ServiceDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                ...
                <ServiceUpdateComponent getServiceList={()=>this.getServiceList()} selectedService={this.state.selectedService}/>
                <ServiceViewComponent  selectedService={this.state.selectedService}
