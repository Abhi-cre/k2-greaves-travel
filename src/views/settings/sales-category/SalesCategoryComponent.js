...
class SalesCategoryComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,salesCategoryListAll:[],salesCategoryListFilter:[],salesCategoryList:[],selectedSalesCategory:{},perPage:8,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(salesCategoryListData.length>0)
     {
        ...
        this.setState({salesCategoryListAll:salesCategoryListData,salesCategoryListFilter:salesCategoryListData,salesCategoryList:salesCategoryList});
     }
     else
     {
    ...
    getSalesCategoryList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/SalesCategory/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'salesCategories')[i],'id')!='') {
                salesCategoryList.push(ArrayHelper.getValue(response,'salesCategories')[i]);
              }
          }
          
          this.setState({loader:false,salesCategoryListAll:ArrayHelper.getValue(response,'salesCategories'),salesCategoryListFilter:ArrayHelper.getValue(response,'salesCategories'),salesCategoryList:salesCategoryList});
          this.props.salesCategoryListInfo(ArrayHelper.getValue(response,'salesCategories'));
        }
        else
        {
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.salesCategoryListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.salesCategoryListFilter[i],'id')!='') {
            salesCategoryList.push(this.state.salesCategoryListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), salesCategoryList: salesCategoryList });
          } else {
        ...
        updatedSalesCategoryList=async(str)=>
        {
            ...
            salesCategoryListFilter=salesCategoryListFilter.map((item)=>{
              ...
              this.setState({salesCategoryListAll:salesCategoryListAll,salesCategoryListFilter:salesCategoryListFilter,salesCategoryList:salesCategoryList,selectedSalesCategory:str});
        ...
        SalesCategoryDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Sales Category?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/SalesCategory/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let salesCategoryListAll=this.state.salesCategoryListAll;
            salesCategoryListAll=salesCategoryListAll.filter((item)=>item.id!=str)
            let salesCategoryListFilter=this.state.salesCategoryListAll;
            salesCategoryListFilter=salesCategoryListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,salesCategoryListAll:salesCategoryListAll,salesCategoryListFilter:salesCategoryListFilter});
              this.props.salesCategoryListInfo(salesCategoryListAll);
        ...
        filterData()
        {
            ...
            if(this.state.keyword.trim()!='')
            {
                ...
                this.setState({salesCategoryListFilter:salesCategoryListFilter,currentPage:1});
      setTimeout(()=>{
        this.setState({loader:false})
        this.props.history("/settings/sales-category"); 
      },10)
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,salesCategoryListFilter:this.state.salesCategoryListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/sales-category"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = salesCategoryListFilter.length > 0 ? salesCategoryListFilter : salesCategoryListAll;
        return(
...
<div className="d-flex align-items-center justify-content-end">
                  ...
                  <button data-bs-toggle="modal" data-bs-target="#salesCategoryAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.salesCategoryList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.salesCategoryList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#salesCategoryUpdate" onClick={()=>{this.setState({selectedSalesCategory:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                                                    <i onClick={()=>this.SalesCategoryDelete(ArrayHelper.getValue(item,'id'))} className="fa fa-trash btn" data-bs-toggle="tooltip" data-bs-placement="top" title="Trash"></i>
                                                </td>
                                            </tr>)
                                        })}
                                    </tbody>
                                ...
                                <p>No Record Found</p>}
                                {(this.state.salesCategoryListFilter.length > this.state.perPage) ?
                    <PaginationComponent
                ...
                <SalesCategoryAddComponent getSalesCategoryList={()=>this.getSalesCategoryList()}/>
                <SalesCategoryUpdateComponent updatedSalesCategoryList={(str)=>this.updatedSalesCategoryList(str)} selectedSalesCategory={this.state.selectedSalesCategory}/>
            </React.Fragment>
        )
    }
}  
...
const mapDispatchToProps = dispatch => {
    return {
        salesCategoryListInfo:(data)=>dispatch({type: actionTypesUser.SALES_CATEGORY_LIST,payload:data}),
       
    }
};

export default  connect(mapStateToProps, mapDispatchToProps)(SalesCategoryComponent);