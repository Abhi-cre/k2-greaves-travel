...
class TourTypeComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {loader:false,tourTypeListAll:[],tourTypeListFilter:[],tourTypeList:[],selectedTourType:{},perPage:20,currentPage:1,keyword:''};
    }
    componentDidMount()
    {
     ...
     if(tourTypeListData.length>0)
     {
        ...
        this.setState({tourTypeListAll:tourTypeListData,tourTypeListFilter:tourTypeListData,tourTypeList:tourTypeList});
     }
     else
     {
    ...
    getTourTypeList=async()=>
    {
      ...
      let response= await SettingApi.GetSettingList('/api/TourType/List');
        if(ArrayHelper.getValue(response,'isSuccess')== true)
        {
          ...
          for (let i = 0; i < this.state.perPage; i++) {
              if (ArrayHelper.getValue(ArrayHelper.getValue(response,'tourTypes')[i],'id')!='') {
                tourTypeList.push(ArrayHelper.getValue(response,'tourTypes')[i]);
              }
          }
          
          this.setState({loader:false,tourTypeListAll:ArrayHelper.getValue(response,'tourTypes'),tourTypeListFilter:ArrayHelper.getValue(response,'tourTypes'),tourTypeList:tourTypeList});
    ...
    componentWillReceiveProps(nextProps) {
        if(this.state.tourTypeListFilter.length>0)
        {
         const location = nextProps.params;
          if (location.page !== undefined && location.page !== this.state.currentPage) {
            ...
            for (i; i < numRecord ; i++) {
          if (ArrayHelper.getValue(this.state.tourTypeListFilter[i],'id')!='') {
            tourTypeList.push(this.state.tourTypeListFilter[i]);
          }
        }
              this.setState({ currentPage: parseInt(location.page), tourTypeList: tourTypeList });
          } else {
        ...
        updatedTourTypeList=async(str)=>
        {
            ...
            tourTypeListFilter=tourTypeListFilter.map((item)=>{
              ...
              this.setState({tourTypeListAll:tourTypeListAll,tourTypeListFilter:tourTypeListFilter,tourTypeList:tourTypeList,selectedTourType:str});
        ...
        SalesRegionDelete=async(str)=>
        {
            if(window.confirm('Do you want to delete selected Tour Type?'))
            {
                ...
                let response= await SettingApi.PostSettingList(formData,'/api/TourType/Delete');
                if(ArrayHelper.getValue(response,'isSuccess')==true)
                {
            let tourTypeListAll=this.state.tourTypeListAll;
            tourTypeListAll=tourTypeListAll.filter((item)=>item.id!=str)
            let tourTypeListFilter=this.state.tourTypeListFilter;
            tourTypeListFilter=tourTypeListFilter.filter((item)=>item.id!=str)
         
              this.setState({loader:false,tourTypeListAll:tourTypeListAll,tourTypeListFilter:tourTypeListFilter});
              this.props.tourTypeListInfo(tourTypeListAll);
      ...
      setTimeout(()=>{
        ...
        clearData()
        {
            this.setState({loader:true,keyword:'',currentPage:1,tourTypeListFilter:this.state.tourTypeListAll})
            setTimeout(()=>{              
             this.setState({loader:false})
             this.props.history("/settings/tour-type"); 
           },10)
    ...
    render(){
        ...
        const dataToExport = tourTypeListFilter.length > 0 ? tourTypeListFilter : tourTypeListAll;
        return(
        ...
        <div className="tab-pane fade show active" id="AgencyType-tab-pane" role="tabpanel" aria-labelledby="AgencyType-tab" tabIndex="0">
                    ...
                    <a  onClick={()=>this.reloadWindow()}><img style={{'height': '40px'}} src="/images/reload.png"/></a>       
                     <button data-bs-toggle="modal" data-bs-target="#tourTypeAdd" className="btn btn-outlined"> <i className="fa-sharp fa-solid fa-plus"></i> Add</button>
                    </div>
...
<div className="d-flex align-items-center justify-content-end">
                  <a onClick={() => this.reloadWindow()}><img style={{ 'height': '40px' }} src="/images/reload.png" /></a>
                  <button data-bs-toggle="modal" data-bs-target="#tourTypeAdd" className="btn btn-outlined me-1">
...
/>
                        ...
                        <div className="borderless-box">
                            <div className="table-responsive">
                               {(this.state.tourTypeList.length>0)?<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>Sr. No</th>
                                            <th>Name</th>                                           
                                    ...
                                    <tbody>
                                        {this.state.tourTypeList.map((item,key)=>{
                                            return( <tr key={`actionType-${key}`}>
                                                ...
                                                <td>
                                                    <i data-bs-toggle="modal" data-bs-target="#tourTypeUpdate" onClick={()=>{this.setState({selectedTourType:item})}}  className="fa fa-edit btn"  data-bs-placement="top" title="Edit"></i>
                                                </td>                                               
                                                <td>
                ...
                <TourTypeUpdateComponent updatedTourTypeList={(str)=>this.updatedTourTypeList(str)} selectedTourType={this.state.selectedTourType}/>
            </React.Fragment>
        )
    }
}  
