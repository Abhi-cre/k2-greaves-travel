...
class MobileUsersComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    if (mobileUserListData.length > 0) {
      ...
      this.setState({
        mobileUserListAll: mobileUserListData,
        mobileUserListFilter: mobileUserListData,
  ...
  getMobileUserList = async () => {
    ...
    if (ArrayHelper.getValue(response, "isSuccess") == true) {
      let mobileUserData = ArrayHelper.getValue(response, "appUsers").map(
        (Item) => {
      ...
      this.setState({
        loader: false,
        mobileUserListAll: mobileUserData,
        mobileUserListFilter: mobileUserData,
        mobileUserList: mobileUserList,
      });
        ...
        for (i; i < numRecord; i++) {
          if (
            ArrayHelper.getValue(
              this.state.mobileUserListFilter[i],
              "travelerID"
            ) != ""
          ) {
  ...
  updatedMobileUserList = async (str) => {
    ...
    this.setState({
      mobileUserListAll: mobileUserListAll,
      mobileUserListFilter: mobileUserListFilter,
      mobileUserList: mobileUserList,
      selectedMobileUser: str,
    });
  ...
  AgentDelete = async (str) => {
    if (window.confirm("Do you want to delete selected Mobile User?")) {
      ...
      if (ArrayHelper.getValue(response, "isSuccess") == true) {
        let mobileUserListAll = this.state.mobileUserListAll;
        mobileUserListAll = mobileUserListAll.filter(
          (item) => item.travelerID != str
        );
        let mobileUserListFilter = this.state.mobileUserListFilter;
        mobileUserListFilter = mobileUserListFilter.filter(
          (item) => item.travelerID != str
        );
  ...
  filterData() {
    ...
    let mobileUserListFilter = mobileUserListAll.map((item) => {
      ...
      if (
        item.company.search(new RegExp(this.state.company.trim(), "i")) == -1 &&
        this.state.company.trim() != ""
      ) {
        item.display = false;
      }
      if (
        item.phone.search(new RegExp(this.state.phone.trim(), "i")) == -1 &&
    ...
    let mobileUserListFilter1 = mobileUserListFilter.filter(
      (item) => item.display == true
    );

    this.setState({
      mobileUserListFilter: mobileUserListFilter1,
      currentPage: 1,
    });
    setTimeout(() => {
      this.setState({ loader: false });
  ...
  clearData() {
    this.setState({
      ...
      phone: "",
      currentPage: 1,
      mobileUserListFilter: this.state.mobileUserListAll,
    });
    setTimeout(() => {
      this.setState({ loader: false });
      this.props.history("/mobile-users");
    ...
    return (
      <React.Fragment>
                    ...
                    <div className="p-4">
                      <div className="row mb-3">
                        ...
                        <div className="col-md-2 text-end">
                          ...
                          ).length > 0 || ISSITESTATUS == "live" ? (
                            <button
                              data-bs-toggle="modal"
                              data-bs-target="#mobileUserAdd"
                      ...
                      <div className="borderless-box">
                        <div className="table-responsive">
                          {this.state.mobileUserList.length > 0 ? (
                                ...
                                {this.state.mobileUserList.map((item, key) => {
                                  return (
                                    <tr key={`Agent-${key}`}>
                                      ...
                                      <td>
                                        {ArrayHelper.getValue(item, "fullName")}
                                      </td>
                                      <td>
                                        {ArrayHelper.getValue(item, "email")}
                                      </td>
                                      <td>
                                        {ArrayHelper.getValue(item, "company")}
                                      </td>
                                      <td>
                                      ...
                                      {ALLOWEDUSER.filter(
                                        ...
                                        <td>
                                          <i
                                            data-bs-toggle="modal"
                                            data-bs-target="#mobileUserUpdate"
                                            onClick={() => {
                                              this.setState({
                                                selectedMobileUser: item,
                                              });
                                            }}
                          ...
                          {this.state.mobileUserListFilter.length >
                          this.state.perPage ? (
                            <PaginationComponent
        ...
        <MobileUserAddComponent
          getMobileUserList={() => this.getMobileUserList()}
        />
        <MobileUserUpdateComponent
          updatedMobileUserList={(str) => this.updatedMobileUserList(str)}
          selectedMobileUser={this.state.selectedMobileUser}
        />
        <MobileUserViewComponent
          selectedMobileUser={this.state.selectedMobileUser}
        />
      </React.Fragment>
    );
...
const mapDispatchToProps = (dispatch) => {
  return {
    mobileUserListInfo: (data) =>
      dispatch({ type: actionTypesUser.MOBILE_USER_LIST, payload: data }),
  };
};
